...
(defnc api-mock [{:keys [enable url id]}]
  (d/div {:class-name "self-stretch py-4 bg-white border-b border-gray-200 justify-center items-center inline-flex" :id id
          :on-mouse-enter (fn [e] (prn :enter e))}
         ...
         (d/div {:class-name " rounded-lg border border-gray-200 justify-center items-center gap-2 flex"}
                (d/svg {:width "10"
                        :height "12"
                        :viewBox "0 0 10 12"
                        :fill "none"}
                ...
                (d/button {:on-click (fn [e] (prn :click e))
                           :class-name "px-3 py-2 text-gray-800 text-sm font-medium leading-[21px]"}
                          "remove"))))
