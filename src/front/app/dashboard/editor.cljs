...
(def validate-moclojer-yaml (atom #(constantly false)))

(defn get-validate-moclojer-yaml-fn []
  (-> (fetch/get "/assets/yaml/moclojer-schema.yaml")
      (p/then
       #(reset! validate-moclojer-yaml (gen-ajv-validate-fn (:body %))))
      (p/catch
       #(do
          (.error js/console "failed to retrieve moclojer schema: " %)
          ;; nested `#` are not allowed
          (reset! validate-moclojer-yaml (fn [] (constantly true)))))))
