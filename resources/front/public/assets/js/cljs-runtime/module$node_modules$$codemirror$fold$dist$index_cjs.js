...
announceFold(view,folded,!1))}effects.length&&view.dispatch({effects});return 0<effects.length};module=view=>{let {state}=view,effects=[];for(var pos=0;pos<state.doc.length;){pos=view.visualLineAt(pos);let range=language.foldable(state,pos.from,pos.to);range&&effects.push(foldEffect.of(range));pos=(range?view.visualLineAt(range.to):pos).to+1}effects.length&&view.dispatch({effects:maybeEnable(view.state,effects)});return!!effects.length};const unfoldAll=view=>{let field=view.state.field(foldState,
!1);if(!field||!field.size)return!1;let effects=[];field.between(0,view.state.doc.length,(from,to)=>{effects.push(unfoldEffect.of({from,to}))});view.dispatch({effects});return!0},foldKeymap=[{key:"Ctrl-Shift-[",mac:"Cmd-Alt-[",run:global},{key:"Ctrl-Shift-]",mac:"Cmd-Alt-]",run:require},{key:"Ctrl-Alt-[",run:module},{key:"Ctrl-Alt-]",run:unfoldAll}],defaultConfig={placeholderDOM:null,placeholderText:"…"},foldConfig=state$jscomp$0.Facet.define({combine(values){return state$jscomp$0.combineConfig(values,
defaultConfig)}}),foldWidget=view$jscomp$0.Decoration.replace({widget:new class extends view$jscomp$0.WidgetType{ignoreEvents(){return!1}toDOM(view){let {state}=view,conf=state.facet(foldConfig);if(conf.placeholderDOM)return conf.placeholderDOM();let element=document.createElement("span");element.textContent=conf.placeholderText;element.setAttribute("aria-label",state.phrase("folded code"));element.title=state.phrase("unfold");element.className="cm-foldPlaceholder";element.onclick=event=>{var line=
view.visualLineAt(view.posAtDOM(event.target));(line=foldInside(view.state,line.from,line.to))&&view.dispatch({effects:unfoldEffect.of(line)});event.preventDefault()};return element}}}),foldGutterDefaults={openText:"⌄",closedText:"›",markerDOM:null};class FoldMarker extends gutter.GutterMarker{constructor(config,open){super();this.config=config;this.open=open}eq(other){return this.config==other.config&&this.open==other.open}toDOM(view){if(this.config.markerDOM)return this.config.markerDOM(this.open);
let span=document.createElement("span");span.textContent=this.open?this.config.openText:this.config.closedText;span.title=view.state.phrase(this.open?"Fold line":"Unfold line");return span}}const baseTheme=view$jscomp$0.EditorView.baseTheme({".cm-foldPlaceholder":{backgroundColor:"#eee",border:"1px solid #ddd",color:"#888",borderRadius:".2em",margin:"0 1px",padding:"0 1px",cursor:"pointer"},".cm-foldGutter .cm-gutterElement":{padding:"0 1px",cursor:"pointer"}});exports.codeFolding=codeFolding;exports.foldAll=
...
update.startState.field(foldState,!1)!=update.state.field(foldState,!1))this.markers=this.buildMarkers(update.view)}buildMarkers(view){let builder=new rangeset.RangeSetBuilder;view.viewportLines(line=>{let mark=foldInside(view.state,line.from,line.to)?canUnfold:language.foldable(view.state,line.from,line.to)?canFold:null;mark&&builder.add(line.from,line.from,mark)});return builder.finish()}});return[markers,gutter.gutter({class:"cm-foldGutter",markers(view){var _a;return(null===(_a=view.plugin(markers))||
void 0===_a?void 0:_a.markers)||rangeset.RangeSet.empty},initialSpacer(){return new FoldMarker(fullConfig,!1)},domEventHandlers:{click:(view,line)=>{let folded=foldInside(view.state,line.from,line.to);return folded?(view.dispatch({effects:unfoldEffect.of(folded)}),!0):(line=language.foldable(view.state,line.from,line.to))?(view.dispatch({effects:foldEffect.of(line)}),!0):!1}}}),codeFolding()]};exports.foldKeymap=foldKeymap;exports.foldedRanges=function(state){return state.field(foldState,!1)||rangeset.RangeSet.empty};
exports.unfoldAll=unfoldAll;exports.unfoldCode=require;exports.unfoldEffect=unfoldEffect}
