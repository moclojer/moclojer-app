...
let parent=node.parentNode;if(!parent||1!=parent.nodeType)return!1;off=domIndex(node)+(0>dir?0:1);node=parent}else if(1==node.nodeType){node=node.childNodes[off+(0>dir?-1:0)];if(1==node.nodeType&&"false"==node.contentEditable)return!1;off=0>dir?maxOffset(node):0}else return!1}}function maxOffset(node){return 3==node.nodeType?node.nodeValue.length:node.childNodes.length}function flattenRect(rect,left){left=left?rect.left:rect.right;return{left,right:left,top:rect.top,bottom:rect.bottom}}function focusPreventScroll(dom){if(dom.setActive)return dom.setActive();
...
else if(view.sliceDoc(cView,changes)!=to)return Decoration.none;return Decoration.set(Decoration.replace({widget:new CompositionWidget(topNode,sel)}).range(cView,changes))}function nearbyTextNode(node,offset,side){for(;;){if(3==node.nodeType)return node;if(1==node.nodeType&&0<offset&&0>=side)node=node.childNodes[offset-1],offset=maxOffset(node);else if(1==node.nodeType&&offset<node.childNodes.length&&0<=side)node=node.childNodes[offset],offset=0;else return null}}function findChangedDeco(a,b,diff){let comp=
new DecorationComparator$1;rangeset.RangeSet.compare(a,b,diff,comp);return comp.changes}function dec(str){let result=[];for(let i=0;i<str.length;i++)result.push(1<<+str[i]);return result}function computeOrder(line,direction){let len=line.length;var outerType=direction==LTR?1:2;direction=direction==LTR?2:1;if(!line||1==outerType&&!BidiRE.test(line))return[new BidiSpan(0,len,0)];for(let i=0,prev=outerType,prevStrong=outerType;i<len;i++){var ch$jscomp$0=line.charCodeAt(i);ch$jscomp$0=247>=ch$jscomp$0?
LowTypes[ch$jscomp$0]:1424<=ch$jscomp$0&&1524>=ch$jscomp$0?2:1536<=ch$jscomp$0&&1785>=ch$jscomp$0?ArabicTypes[ch$jscomp$0-1536]:1774<=ch$jscomp$0&&2220>=ch$jscomp$0?4:8192<=ch$jscomp$0&&8203>=ch$jscomp$0?256:8204==ch$jscomp$0?256:1;512==ch$jscomp$0?ch$jscomp$0=prev:8==ch$jscomp$0&&4==prevStrong&&(ch$jscomp$0=16);types[i]=4==ch$jscomp$0?2:ch$jscomp$0;ch$jscomp$0&7&&(prevStrong=ch$jscomp$0);prev=ch$jscomp$0}for(let i=0,prev=outerType,prevStrong=outerType;i<len;i++){ch$jscomp$0=types[i];if(128==ch$jscomp$0)i<
len-1&&prev==types[i+1]&&prev&24?ch$jscomp$0=types[i]=prev:types[i]=256;else if(64==ch$jscomp$0){for(var end=i+1;end<len&&64==types[end];)end++;var replace=i&&8==prev||end<len&&8==types[end]?1==prevStrong?1:8:256;for(let j=i;j<end;j++)types[j]=replace;i=end-1}else 8==ch$jscomp$0&&1==prevStrong&&(types[i]=1);prev=ch$jscomp$0;ch$jscomp$0&7&&(prevStrong=ch$jscomp$0)}for(let i=0,sI=0,context=0,ch,br,type;i<len;i++)if(br=Brackets[ch=line.charCodeAt(i)])if(0>br)for(ch$jscomp$0=sI-3;0<=ch$jscomp$0;ch$jscomp$0-=
3){if(BracketStack[ch$jscomp$0+1]==-br){end=BracketStack[ch$jscomp$0+2];(end=end&2?outerType:end&4?end&1?direction:outerType:0)&&(types[i]=types[BracketStack[ch$jscomp$0]]=end);sI=ch$jscomp$0;break}}else if(189==BracketStack.length)break;else BracketStack[sI++]=i,BracketStack[sI++]=ch,BracketStack[sI++]=context;else if(2==(type=types[i])||1==type)for(context=(ch$jscomp$0=type==outerType)?0:1,end=sI-3;0<=end;end-=3){replace=BracketStack[end+2];if(replace&2)break;if(ch$jscomp$0)BracketStack[end+2]|=
2;else{if(replace&4)break;BracketStack[end+2]|=4}}for(direction=0;direction<len;direction++)if(256==types[direction]){for(line=direction+1;line<len&&256==types[line];)line++;ch$jscomp$0=1==(direction?types[direction-1]:outerType);for(ch$jscomp$0=ch$jscomp$0==(1==(line<len?types[line]:outerType))?ch$jscomp$0?1:2:outerType;direction<line;direction++)types[direction]=ch$jscomp$0;direction=line-1}line=[];if(1==outerType)for(outerType=0;outerType<len;){direction=outerType;for(ch$jscomp$0=1!=types[outerType++];outerType<
len&&ch$jscomp$0==(1!=types[outerType]);)outerType++;if(ch$jscomp$0)for(ch$jscomp$0=outerType;ch$jscomp$0>direction;){end=ch$jscomp$0;for(replace=2!=types[--ch$jscomp$0];ch$jscomp$0>direction&&replace==(2!=types[ch$jscomp$0-1]);)ch$jscomp$0--;line.push(new BidiSpan(ch$jscomp$0,end,replace?2:1))}else line.push(new BidiSpan(direction,outerType,0))}else for(outerType=0;outerType<len;){direction=outerType;for(ch$jscomp$0=2==types[outerType++];outerType<len&&ch$jscomp$0==(2==types[outerType]);)outerType++;
line.push(new BidiSpan(direction,outerType,ch$jscomp$0?1:2))}return line}function moveVisually(line,order,dir,start,forward){var _a;let startIndex=start.head-line.from,spanI=-1;if(0==startIndex){if(!forward||!line.length)return null;order[0].level!=dir&&(startIndex=order[0].side(!1,dir),spanI=0)}else if(startIndex==line.length){if(forward)return null;var last=order[order.length-1];last.level!=dir&&(startIndex=last.side(!0,dir),spanI=order.length-1)}0>spanI&&(spanI=BidiSpan.find(order,startIndex,null!==
(_a=start.bidiLevel)&&void 0!==_a?_a:-1,start.assoc));start=order[spanI];startIndex==start.side(forward,dir)&&(start=order[spanI+=forward?1:-1],startIndex=start.side(!forward,dir));last=forward==(start.dir==dir);_a=text$jscomp$0.findClusterBreak(line.text,startIndex,last);movedOver=line.text.slice(Math.min(startIndex,_a),Math.max(startIndex,_a));return _a!=start.side(forward,dir)?state$jscomp$0.EditorSelection.cursor(_a+line.from,last?-1:1,start.level):(order=spanI==(forward?order.length-1:0)?null:
order[spanI+(forward?1:-1)])||start.level==dir?order&&order.level<start.level?state$jscomp$0.EditorSelection.cursor(order.side(!forward,dir)+line.from,forward?1:-1,order.level):state$jscomp$0.EditorSelection.cursor(_a+line.from,forward?-1:1,start.level):state$jscomp$0.EditorSelection.cursor(forward?line.to:line.from,forward?-1:1,dir)}function groupAt(state$1,pos,bias=1){let categorize=state$1.charCategorizer(pos);state$1=state$1.doc.lineAt(pos);var linePos=pos-state$1.from;if(0==state$1.length)return state$jscomp$0.EditorSelection.cursor(pos);
...
1:0);closestDY=dy}}}return{node,offset:-1<closestOffset?closestOffset:0<generalSide?node.nodeValue.length:0}}function posAtCoords(view,{x,y},precise,bias=-1){var content=view.contentDOM.getBoundingClientRect();let halfLine=view.defaultLineHeight/2;for(let bounced=!1;;){var block=view.blockAtHeight(y,content.top);if(block.top>y||block.bottom<y){bias=block.top>y?-1:1;y=Math.min(block.bottom-halfLine,Math.max(block.top+halfLine,y));if(bounced)return precise?null:0;bounced=!0}if(block.type==exports.BlockType.Text)break;
y=0<bias?block.bottom+halfLine:block.top-halfLine}precise=block.from;x=Math.max(content.left+1,Math.min(content.right-1,x));if(precise<view.viewport.from)return 0==view.viewport.from?0:posAtCoordsImprecise(view,content,block,x,y);if(precise>view.viewport.to)return view.viewport.to==view.state.doc.length?view.state.doc.length:posAtCoordsImprecise(view,content,block,x,y);block=view.root;bias=block.elementFromPoint(x,y);var node;content=-1;if(bias&&view.contentDOM.contains(bias)&&!(view.docView.nearest(bias)instanceof
WidgetView))if(block.caretPositionFromPoint){var pos=block.caretPositionFromPoint(x,y);pos&&({offsetNode:node,offset:content}=pos)}else if(block.caretRangeFromPoint&&(block=block.caretRangeFromPoint(x,y))){({startContainer:node,startOffset:content}=block);if(block=browser.safari)a:if(block=x,3!=node.nodeType||content!=(pos=node.nodeValue.length))block=!1;else{for(bias=node.nextSibling;bias;bias=bias.nextSibling)if(1!=bias.nodeType||"BR"!=bias.nodeName){block=!1;break a}block=textRange(node,pos-1,
pos).getBoundingClientRect().left>block}block&&(node=void 0)}node&&view.docView.dom.contains(node)||(node=LineView.find(view.docView,precise),{node,offset:content}=domPosAtCoords(node.dom,x,y));return view.docView.posFromDOM(node,content)}function posAtCoordsImprecise(view,contentRect,block,x,y){contentRect=Math.round((x-contentRect.left)*view.defaultCharacterWidth);view.lineWrapping&&block.height>1.5*view.defaultLineHeight&&(contentRect+=Math.floor((y-block.top)/view.defaultLineHeight)*view.viewState.heightOracle.lineLength);
y=view.state.sliceDoc(block.from,block.to);return block.from+text$jscomp$0.findColumn(y,contentRect,view.state.tabSize)}function moveByChar(view,start,forward,by){let line=view.state.doc.lineAt(start.head),spans=view.bidiSpans(line);for(let cur=start,check=null;;){start=moveVisually(line,spans,view.textDirection,cur,forward);let char=movedOver;if(!start){if(line.number==(forward?view.state.doc.lines:1))return cur;char="\n";line=view.state.doc.line(line.number+(forward?1:-1));spans=view.bidiSpans(line);
start=state$jscomp$0.EditorSelection.cursor(forward?line.from:line.to)}if(!check){if(!by)return start;check=by(char)}else if(!check(char))return cur;cur=start}}function byGroup(view,pos,start){let categorize=view.state.charCategorizer(pos),cat=categorize(start);return next=>{next=categorize(next);cat==state$jscomp$0.CharCategory.Space&&(cat=next);return cat==next}}function skipAtoms(view,oldPos,pos){for(view=view.pluginField(PluginField.atomicRanges);;){let moved=!1;for(let set of view)set.between(pos.from-
1,pos.from+1,(from,to,value)=>{pos.from>from&&pos.from<to&&(pos=oldPos.from>pos.from?state$jscomp$0.EditorSelection.cursor(from,1):state$jscomp$0.EditorSelection.cursor(to,-1),moved=!0)});if(!moved)return pos}}function isInPrimarySelection(view,event){var {main}=view.state.selection;if(main.empty)return!1;view=getSelection(view.root);if(0==view.rangeCount)return!0;view=view.getRangeAt(0).getClientRects();for(main=0;main<view.length;main++){let rect=view[main];if(rect.left<=event.clientX&&rect.right>=
event.clientX&&rect.top<=event.clientY&&rect.bottom>=event.clientY)return!0}return!1}function eventBelongsToEditor(view,event){if(!event.bubbles)return!0;if(event.defaultPrevented)return!1;for(let node=event.target,cView;node!=view.contentDOM;node=node.parentNode)if(!node||11==node.nodeType||(cView=ContentView.get(node))&&cView.ignoreEvent(event))return!1;return!0}function capturePaste(view){let parent=view.dom.parentNode;if(parent){var target=parent.appendChild(document.createElement("textarea"));
target.style.cssText="position: fixed; left: -10000px; top: 10px";target.focus();setTimeout(()=>{view.focus();target.remove();doPaste(view,target.value)},50)}}function doPaste(view,input){let {state:state$1}=view,changes,i=1,text=state$1.toText(input),byLine=text.lines==state$1.selection.ranges.length;if(lastLinewiseCopy&&state$1.selection.ranges.every(r=>r.empty)&&lastLinewiseCopy==text.toString()){let lastLine=-1;changes=state$1.changeByRange(range=>{let line=state$1.doc.lineAt(range.from);if(line.from==
lastLine)return{range};lastLine=line.from;let insert=state$1.toText((byLine?text.line(i++).text:input)+state$1.lineBreak);return{changes:{from:line.from,insert},range:state$jscomp$0.EditorSelection.cursor(range.from+insert.length)}})}else changes=byLine?state$1.changeByRange(range=>{let line=text.line(i++);return{changes:{from:range.from,to:range.to,insert:line.text},range:state$jscomp$0.EditorSelection.cursor(range.from+line.length)}}):state$1.replaceSelection(text);view.dispatch(changes,{annotations:state$jscomp$0.Transaction.userEvent.of("paste"),
scrollIntoView:!0})}function rangeForClick(view,pos,bias,type){if(1==type)return state$jscomp$0.EditorSelection.cursor(pos,bias);if(2==type)return groupAt(view.state,pos,bias);bias=LineView.find(view.docView,pos);pos=view.state.doc.lineAt(bias?bias.posAtEnd:pos);type=bias?bias.posAtStart:pos.from;bias=bias?bias.posAtEnd:pos.to;bias<view.state.doc.length&&bias==pos.to&&bias++;return state$jscomp$0.EditorSelection.range(type,bias)}function queryPos(view,event){let pos=view.posAtCoords({x:event.clientX,
y:event.clientY},!1);{var JSCompiler_inline_result=event.clientX;event=event.clientY;let line=LineView.find(view.docView,pos);if(line){var off=pos-line.posAtStart;if(0==off)JSCompiler_inline_result=1;else if(off==line.length)JSCompiler_inline_result=-1;else{view=line.coordsAt(off,-1);var JSCompiler_temp;if(JSCompiler_temp=view)JSCompiler_temp=event>=view.top&&event<=view.bottom&&JSCompiler_inline_result>=view.left&&JSCompiler_inline_result<=view.right;if(JSCompiler_temp)JSCompiler_inline_result=-1;
else{if(JSCompiler_temp=off=line.coordsAt(off,1))JSCompiler_temp=event>=off.top&&event<=off.bottom&&JSCompiler_inline_result>=off.left&&JSCompiler_inline_result<=off.right;JSCompiler_inline_result=JSCompiler_temp?1:view&&event>=view.top&&event<=view.bottom?-1:1}}}else JSCompiler_inline_result=1}return{pos,bias:JSCompiler_inline_result}}function getClickType(event){if(!BadMouseDetail)return event.detail;let last=lastMouseDown,lastTime=lastMouseDownTime;lastMouseDown=event;lastMouseDownTime=Date.now();
return lastMouseDownCount=!last||lastTime>Date.now()-400&&2>Math.abs(last.clientX-event.clientX)&&2>Math.abs(last.clientY-event.clientY)?(lastMouseDownCount+1)%3:1}function basicMouseSelection(view,event$jscomp$0){let start=queryPos(view,event$jscomp$0),type=getClickType(event$jscomp$0),startSel=view.state.selection,last=start,lastEvent=event$jscomp$0;return{update(update){update.changes&&(start&&(start.pos=update.changes.mapPos(start.pos)),startSel=startSel.map(update.changes),lastEvent=null)},get(event,
extend,multiple){if(lastEvent&&event.clientX==lastEvent.clientX&&event.clientY==lastEvent.clientY)var cur=last;else cur=last=queryPos(view,event),lastEvent=event;if(!cur||!start)return startSel;event=rangeForClick(view,cur.pos,cur.bias,type);if(start.pos!=cur.pos&&!extend){var startRange=rangeForClick(view,start.pos,start.bias,type);cur=Math.min(startRange.from,event.from);startRange=Math.max(startRange.to,event.to);event=cur<event.from?state$jscomp$0.EditorSelection.range(cur,startRange):state$jscomp$0.EditorSelection.range(startRange,
cur)}return extend?startSel.replaceRange(startSel.main.extend(event.from,event.to)):multiple?startSel.addRange(event):state$jscomp$0.EditorSelection.create([event])}}}function dropText(view,event,text,direct){let dropPos=view.posAtCoords({x:event.clientX,y:event.clientY});null!=dropPos&&text&&(event.preventDefault(),{mouseSelection:event}=view.inputState,direct=direct&&event&&event.dragging&&event.dragMove?{from:event.dragging.from,to:event.dragging.to}:null,text={from:dropPos,insert:text},text=view.state.changes(direct?
[direct,text]:text),view.focus(),view.dispatch({changes:text,selection:{anchor:text.mapPos(dropPos,-1),head:text.mapPos(dropPos,1)},annotations:state$jscomp$0.Transaction.userEvent.of("drop")}))}function captureCopy(view,text){let parent=view.dom.parentNode;if(parent){var target=parent.appendChild(document.createElement("textarea"));target.style.cssText="position: fixed; left: -10000px; top: 10px";target.value=text;target.focus();target.selectionEnd=text.length;target.selectionStart=0;setTimeout(()=>
{target.remove();view.focus()},50)}}function forceClearComposition(view,rapid){if(view.docView.compositionDeco.size){view.inputState.rapidCompositionStart=rapid;try{view.update([])}finally{view.inputState.rapidCompositionStart=!1}}}function mergeGaps(nodes,around){let before,after;null==nodes[around]&&(before=nodes[around-1])instanceof HeightMapGap&&(after=nodes[around+1])instanceof HeightMapGap&&nodes.splice(around-1,3,new HeightMapGap(before.length+1+after.length))}function lineStructure(from$jscomp$0,
to$jscomp$0,state){let ranges=[],pos=from$jscomp$0,total=0;rangeset.RangeSet.spans(state.facet(decorations),from$jscomp$0,to$jscomp$0,{span(){},point(from,to){from>pos&&(ranges.push({from:pos,to:from}),total+=from-pos);pos=to}},20);pos<to$jscomp$0&&(ranges.push({from:pos,to:to$jscomp$0}),total+=to$jscomp$0-pos);return{total,ranges}}function findPosition({total,ranges},ratio){if(0>=ratio)return ranges[0].from;if(1<=ratio)return ranges[ranges.length-1].to;total=Math.floor(total*ratio);for(ratio=0;;ratio++){let {from,
to}=ranges[ratio],size=to-from;if(total<=size)return from+total;total-=size}}function findFraction(structure,pos){let counted=0;for(let {from,to}of structure.ranges){if(pos<=to){counted+=pos-from;break}counted+=to-from}return counted/structure.total}function find(array,f){for(let val of array)if(f(val))return val}function scaleBlock(block,scaler,top){if(1==scaler.scale)return block;let bTop=scaler.toDOM(block.top,top),bBottom=scaler.toDOM(block.bottom,top);return new BlockInfo(block.from,block.length,
bTop,bBottom-bTop,Array.isArray(block.type)?block.type.map(b=>scaleBlock(b,scaler,top)):block.type)}function buildTheme(main,spec,scopes){return new styleMod.StyleModule(spec,{finish(sel){return/&/.test(sel)?sel.replace(/&\w*/,m=>{if("\x26"==m)return main;if(!scopes||!scopes[m])throw new RangeError(`Unsupported selector: ${m}`);return scopes[m]}):main+" "+sel}})}function findChild(cView,dom,dir){for(;dom;){var curView=ContentView.get(dom);if(curView&&curView.parent==cView)return curView;curView=dom.parentNode;
dom=curView!=cView.dom?curView:0<dir?dom.nextSibling:dom.previousSibling}return null}function safariSelectionRangeHack(view){function read(event){event.preventDefault();event.stopImmediatePropagation();found=event.getTargetRanges()[0]}let found=null;view.contentDOM.addEventListener("beforeinput",read,!0);document.execCommand("indent");view.contentDOM.removeEventListener("beforeinput",read,!0);if(!found)return null;let anchorNode=found.startContainer,anchorOffset=found.startOffset,focusNode=found.endContainer,
...
view.docView.impreciseHead||view.docView.impreciseAnchor?[]:selectionPoints(view);start=new DOMReader(newSel,view);start.readRange(bounds.startDOM,bounds.endDOM);newSel=selectionFromPoints(newSel,from);bounds=sel.from;end=null;if(8===view.inputState.lastKeyCode&&view.inputState.lastKeyTime>Date.now()-100||browser.android&&start.text.length<to-from)bounds=sel.to,end="end";(bounds=findDiff(view.state.sliceDoc(from,to),start.text,bounds-from,end))&&(change={from:from+bounds.from,to:from+bounds.toA,insert:view.state.toText(start.text.slice(bounds.from,
bounds.toB))})}else if(view.hasFocus||!view.state.facet(editable)){bounds=view.observer.selectionRange;let {impreciseHead:iHead,impreciseAnchor:iAnchor}=view.docView;start=iHead&&iHead.node==bounds.focusNode&&iHead.offset==bounds.focusOffset||!contains(view.contentDOM,bounds.focusNode)?view.state.selection.main.head:view.docView.posFromDOM(bounds.focusNode,bounds.focusOffset);bounds=iAnchor&&iAnchor.node==bounds.anchorNode&&iAnchor.offset==bounds.anchorOffset||!contains(view.contentDOM,bounds.anchorNode)?
view.state.selection.main.anchor:view.docView.posFromDOM(bounds.anchorNode,bounds.anchorOffset);if(start!=sel.head||bounds!=sel.anchor)newSel=state$jscomp$0.EditorSelection.single(bounds,start)}if(change||newSel)if(!change&&typeOver&&!sel.empty&&newSel&&newSel.main.empty?change={from:sel.from,to:sel.to,insert:view.state.doc.slice(sel.from,sel.to)}:change&&change.from>=sel.from&&change.to<=sel.to&&(change.from!=sel.from||change.to!=sel.to)&&4>=sel.to-sel.from-(change.to-change.from)&&(change={from:sel.from,
to:sel.to,insert:view.state.doc.slice(sel.from,change.from).append(change.insert).append(view.state.doc.slice(change.to,sel.to))}),change){var startState=view.state;if(!(browser.android&&(change.from==sel.from&&change.to==sel.to&&1==change.insert.length&&2==change.insert.lines&&dispatchKey(view.contentDOM,"Enter",13)||change.from==sel.from-1&&change.to==sel.to&&0==change.insert.length&&dispatchKey(view.contentDOM,"Backspace",8)||change.from==sel.from&&change.to==sel.to+1&&0==change.insert.length&&
dispatchKey(view.contentDOM,"Delete",46))||browser.ios&&view.inputState.flushIOSKey(view))){var text=change.insert.toString();view.state.facet(inputHandler).some(h=>h(view,change.from,change.to,text))||(0<=view.inputState.composing&&view.inputState.composing++,change.from>=sel.from&&change.to<=sel.to&&change.to-change.from>=(sel.to-sel.from)/3&&(!newSel||newSel.main.empty&&newSel.main.from==change.from+change.insert.length)?(newSel=sel.from<change.from?startState.sliceDoc(sel.from,change.from):"",
sel=sel.to>change.to?startState.sliceDoc(change.to,sel.to):"",newSel=startState.replaceSelection(view.state.toText(newSel+change.insert.sliceString(0,void 0,view.state.lineBreak)+sel))):(sel=startState.changes(change),newSel={changes:sel,selection:newSel&&!startState.selection.main.eq(newSel.main)&&newSel.main.to<=sel.newLength?startState.selection.replaceRange(newSel.main):void 0}),view.dispatch(newSel,{scrollIntoView:!0,annotations:state$jscomp$0.Transaction.userEvent.of("input")}))}}else newSel&&
!newSel.main.eq(sel)&&(sel=!1,view.inputState.lastSelectionTime>Date.now()-50&&("keyboardselection"==view.inputState.lastSelectionOrigin?sel=!0:startState=state$jscomp$0.Transaction.userEvent.of(view.inputState.lastSelectionOrigin)),view.dispatch({selection:newSel,scrollIntoView:sel,annotations:startState}))}function findDiff(a,b,preferredPos,preferredSide){var minLen=Math.min(a.length,b.length);let from=0;for(;from<minLen&&a.charCodeAt(from)==b.charCodeAt(from);)from++;if(from==minLen&&a.length==
b.length)return null;minLen=a.length;let toB=b.length;for(;0<minLen&&0<toB&&a.charCodeAt(minLen-1)==b.charCodeAt(toB-1);)minLen--,toB--;"end"==preferredSide&&(preferredPos-=minLen+Math.max(0,from-Math.min(minLen,toB))-from);minLen<from&&a.length<b.length?(from-=preferredPos<=from&&preferredPos>=minLen?from-preferredPos:0,toB=from+(toB-minLen),minLen=from):toB<from&&(from-=preferredPos<=from&&preferredPos>=toB?from-preferredPos:0,minLen=from+(minLen-toB),toB=from);return{from,toA:minLen,toB}}function isBlockElement(node){return 1==
node.nodeType&&/^(DIV|P|LI|UL|OL|BLOCKQUOTE|DD|DT|H\d|SECTION|PRE)$/.test(node.nodeName)}function selectionPoints(view){let result=[];if(view.root.activeElement!=view.contentDOM)return result;let {anchorNode,anchorOffset,focusNode,focusOffset}=view.observer.selectionRange;anchorNode&&(result.push(new DOMPoint(anchorNode,anchorOffset)),focusNode==anchorNode&&focusOffset==anchorOffset||result.push(new DOMPoint(focusNode,focusOffset)));return result}function selectionFromPoints(points,base){if(0==points.length)return null;
let anchor=points[0].pos;points=2==points.length?points[1].pos:anchor;return-1<anchor&&-1<points?state$jscomp$0.EditorSelection.single(anchor+base,points+base):null}function ensureTop(given,dom){return null==given?dom.getBoundingClientRect().top:given}function ensureGlobalHandler(){window.addEventListener("resize",()=>{-1==resizeDebounce&&(resizeDebounce=setTimeout(handleResize,50))})}function handleResize(){resizeDebounce=-1;let found=document.querySelectorAll(".cm-content");for(let i=0;i<found.length;i++){let docView=
ContentView.get(found[i]);docView&&docView.editorView.requestMeasure()}}function modifiers(name,event,shift){event.altKey&&(name="Alt-"+name);event.ctrlKey&&(name="Ctrl-"+name);event.metaKey&&(name="Meta-"+name);!1!==shift&&event.shiftKey&&(name="Shift-"+name);return name}function getKeymap(state){state=state.facet(keymap);let map=Keymaps.get(state);map||Keymaps.set(state,map=buildKeymap(state.reduce((a,b)=>a.concat(b),[])));return map}function buildKeymap(bindings,platform=currentPlatform){let bound=
Object.create(null),isPrefix=Object.create(null),checkPrefix=(name,is)=>{let current=isPrefix[name];if(null==current)isPrefix[name]=is;else if(current!=is)throw Error("Key binding "+name+" is used both as a regular binding and as a multi-stroke prefix");},add=(scope,key,command,preventDefault)=>{var scopeObj=bound[scope]||(bound[scope]=Object.create(null));key=key.split(/ (?!$)/).map(k=>{k=k.split(/-(?!$)/);let result=k[k.length-1];"Space"==result&&(result=" ");let alt,ctrl,shift,meta;for(let i=0;i<
k.length-1;++i){const mod=k[i];if(/^(cmd|meta|m)$/i.test(mod))meta=!0;else if(/^a(lt)?$/i.test(mod))alt=!0;else if(/^(c|ctrl|control)$/i.test(mod))ctrl=!0;else if(/^s(hift)?$/i.test(mod))shift=!0;else if(/^mod$/i.test(mod))"mac"==platform?meta=!0:ctrl=!0;else throw Error("Unrecognized modifier name: "+mod);}alt&&(result="Alt-"+result);ctrl&&(result="Ctrl-"+result);meta&&(result="Meta-"+result);shift&&(result="Shift-"+result);return result});for(let i=1;i<key.length;i++){let prefix=key.slice(0,i).join(" ");
checkPrefix(prefix,!0);scopeObj[prefix]||(scopeObj[prefix]={preventDefault:!0,commands:[view=>{let ourObj=storedPrefix={view,prefix,scope};setTimeout(()=>{storedPrefix==ourObj&&(storedPrefix=null)},4E3);return!0}]})}key=key.join(" ");checkPrefix(key,!1);scopeObj=scopeObj[key]||(scopeObj[key]={preventDefault:!1,commands:[]});scopeObj.commands.push(command);preventDefault&&(scopeObj.preventDefault=!0)};for(let b of bindings)if(bindings=b[platform]||b.key)for(let scope of b.scope?b.scope.split(" "):
["editor"])add(scope,bindings,b.run,b.preventDefault),b.shift&&add(scope,"Shift-"+bindings,b.shift,b.preventDefault);return bound}function runHandlers(map,event,view,scope){let name=w3cKeyname.keyName(event),isChar=1==name.length&&" "!=name,prefix="",fallthrough=!1;storedPrefix&&storedPrefix.view==view&&storedPrefix.scope==scope&&(prefix=storedPrefix.prefix+" ",fallthrough=0>modifierCodes.indexOf(event.keyCode))&&(storedPrefix=null);let runFor=binding=>{if(binding){for(let cmd of binding.commands)if(cmd(view))return!0;
...
return{left:(view.textDirection==exports.Direction.LTR?rect.left:rect.right-view.scrollDOM.clientWidth)-view.scrollDOM.scrollLeft,top:rect.top-view.scrollDOM.scrollTop}}function wrappedLine(view,pos,inside){pos=state$jscomp$0.EditorSelection.cursor(pos);return{from:Math.max(inside.from,view.moveToLineBoundary(pos,!1,!0).from),to:Math.min(inside.to,view.moveToLineBoundary(pos,!0,!0).from),type:exports.BlockType.Text}}function blockAt(view,pos){view=view.visualLineAt(pos);if(Array.isArray(view.type))for(let l of view.type)if(l.to>
pos||l.to==pos&&(l.to==view.to||l.type==exports.BlockType.Text))return l;return view}function measureRange(view,range){function piece(left,top,right,bottom){return new Piece(left-base.left,top-base.top,right-left,bottom-top,"cm-selectionBackground")}function pieces({top,bottom,horizontal}){let pieces=[];for(let i=0;i<horizontal.length;i+=2)pieces.push(piece(horizontal[i],top,horizontal[i+1],bottom));return pieces}function drawForLine(from$jscomp$0,to$jscomp$0,line){function addSpan(from,fromOpen,
to,toOpen,dir){from=view.coordsAtPos(from,from==line.to?-2:2);to=view.coordsAtPos(to,to==line.from?2:-2);top=Math.min(from.top,to.top,top);bottom=Math.max(from.bottom,to.bottom,bottom);dir==exports.Direction.LTR?horizontal.push(ltr&&fromOpen?leftSide:from.left,ltr&&toOpen?rightSide:to.right):horizontal.push(!ltr&&toOpen?leftSide:to.left,!ltr&&fromOpen?rightSide:from.right)}let top=1E9,bottom=-1E9,horizontal=[],start=null!==from$jscomp$0&&void 0!==from$jscomp$0?from$jscomp$0:line.from,end=null!==to$jscomp$0&&
void 0!==to$jscomp$0?to$jscomp$0:line.to;for(let r of view.visibleRanges)if(r.to>start&&r.from<end)for(let pos=Math.max(r.from,start),endPos=Math.min(r.to,end);;){let docLine=view.state.doc.lineAt(pos);for(let span of view.bidiSpans(docLine)){let spanFrom=span.from+docLine.from,spanTo=span.to+docLine.from;if(spanFrom>=endPos)break;spanTo>pos&&addSpan(Math.max(spanFrom,pos),null==from$jscomp$0&&spanFrom<=start,Math.min(spanTo,endPos),null==to$jscomp$0&&spanTo>=end,span.dir)}pos=docLine.to+1;if(pos>=
endPos)break}0==horizontal.length&&addSpan(start,null==from$jscomp$0,end,null==to$jscomp$0,view.textDirection);return{top,bottom,horizontal}}function drawForWidget(block,top){block=contentRect.top+(top?block.top:block.bottom);return{top:block,bottom:block,horizontal:[]}}if(range.to<=view.viewport.from||range.from>=view.viewport.to)return[];var from$jscomp$1=Math.max(range.from,view.viewport.from),to$jscomp$1=Math.min(range.to,view.viewport.to);let ltr=view.textDirection==exports.Direction.LTR;var content=
view.contentDOM;let contentRect=content.getBoundingClientRect(),base=getBase(view);content=window.getComputedStyle(content.firstChild);let leftSide=contentRect.left+parseInt(content.paddingLeft),rightSide=contentRect.right-parseInt(content.paddingRight);content=blockAt(view,from$jscomp$1);let endBlock=blockAt(view,to$jscomp$1),visualStart=content.type==exports.BlockType.Text?content:null,visualEnd=endBlock.type==exports.BlockType.Text?endBlock:null;view.lineWrapping&&(visualStart&&(visualStart=wrappedLine(view,
from$jscomp$1,visualStart)),visualEnd&&(visualEnd=wrappedLine(view,to$jscomp$1,visualEnd)));if(visualStart&&visualEnd&&visualStart.from==visualEnd.from)return pieces(drawForLine(range.from,range.to,visualStart));from$jscomp$1=visualStart?drawForLine(range.from,null,visualStart):drawForWidget(content,!1);range=visualEnd?drawForLine(null,range.to,visualEnd):drawForWidget(endBlock,!0);to$jscomp$1=[];(visualStart||content).to<(visualEnd||endBlock).from-1?to$jscomp$1.push(piece(leftSide,from$jscomp$1.bottom,
rightSide,range.top)):from$jscomp$1.bottom<range.top&&blockAt(view,(from$jscomp$1.bottom+range.top)/2).type==exports.BlockType.Text&&(from$jscomp$1.bottom=range.top=(from$jscomp$1.bottom+range.top)/2);return pieces(from$jscomp$1).concat(to$jscomp$1).concat(pieces(range))}function measureCursor(view,cursor,primary){cursor=view.coordsAtPos(cursor.head,cursor.assoc||1);if(!cursor)return null;view=getBase(view);return new Piece(cursor.left-view.left,cursor.top-view.top,-1,cursor.bottom-cursor.top,primary?
"cm-cursor cm-cursor-primary":"cm-cursor cm-cursor-secondary")}function iterMatches(doc,re,from,to,f){re.lastIndex=0;for(let cursor=doc.iterRange(from,to),pos=from,m;!cursor.next().done;pos+=cursor.value.length)if(!cursor.lineBreak)for(;m=re.exec(cursor.value);)f(pos+m.index,pos+m.index+m[0].length,m)}function specialCharPlugin(){return _plugin||(_plugin=ViewPlugin.fromClass(class{constructor(view){this.view=view;this.decorations=Decoration.none;this.decorationCache=Object.create(null);this.decorator=
this.makeDecorator(view.state.facet(specialCharConfig));this.decorations=this.decorator.createDeco(view)}makeDecorator(conf){return new MatchDecorator({regexp:conf.specialChars,decoration:(m,view,pos)=>{let {doc}=view.state;m=text$jscomp$0.codePointAt(m[0],0);return 9==m?(m=doc.lineAt(pos),view=view.state.tabSize,pos=text$jscomp$0.countColumn(doc.sliceString(m.from,pos),0,view),Decoration.replace({widget:new TabWidget((view-pos%view)*this.view.defaultCharacterWidth)})):this.decorationCache[m]||(this.decorationCache[m]=
Decoration.replace({widget:new SpecialCharWidget(conf,m)}))},boundary:conf.replaceTabs?void 0:/[^]/})}update(update){let conf=update.state.facet(specialCharConfig);update.startState.facet(specialCharConfig)!=conf?(this.decorator=this.makeDecorator(conf),this.decorations=this.decorator.createDeco(update.view)):this.decorations=this.decorator.updateDeco(update,this.decorations)}},{decorations:v=>v.decorations}))}Object.defineProperty(exports,"__esModule",{value:!0});var state$jscomp$0=require("module$node_modules$$codemirror$state$dist$index_cjs"),
styleMod=require("module$node_modules$style_mod$dist$style_mod_cjs"),rangeset=require("module$node_modules$$codemirror$rangeset$dist$index_cjs"),text$jscomp$0=require("module$node_modules$$codemirror$text$dist$index_cjs"),w3cKeyname=require("module$node_modules$w3c_keyname$index_cjs");const Rect0={left:0,right:0,top:0,bottom:0};class DOMSelection{constructor(){this.anchorNode=null;this.anchorOffset=0;this.focusNode=null;this.focusOffset=0}eq(domSel){return this.anchorNode==domSel.anchorNode&&this.anchorOffset==
domSel.anchorOffset&&this.focusNode==domSel.focusNode&&this.focusOffset==domSel.focusOffset}set(domSel){this.anchorNode=domSel.anchorNode;this.anchorOffset=domSel.anchorOffset;this.focusNode=domSel.focusNode;this.focusOffset=domSel.focusOffset}}let preventScrollSupported=null,scratchRange,_plainTextSupported=null;class DOMPos{constructor(node,offset,precise=!0){this.node=node;this.offset=offset;this.precise=precise}static before(dom,precise){return new DOMPos(dom.parentNode,domIndex(dom),precise)}static after(dom,
precise){return new DOMPos(dom.parentNode,domIndex(dom)+1,precise)}}const none$3=[];class ContentView{constructor(){this.dom=this.parent=null;this.dirty=2}get editorView(){if(!this.parent)throw Error("Accessing view in orphan content view");return this.parent.editorView}get overrideDOMText(){return null}get posAtStart(){return this.parent?this.parent.posBefore(this):0}get posAtEnd(){return this.posAtStart+this.length}posBefore(view){let pos=this.posAtStart;for(let child of this.children){if(child==
view)return pos;pos+=child.length+child.breakAfter}throw new RangeError("Invalid child in posBefore");}posAfter(view){return this.posBefore(view)+view.length}coordsAt(_pos,_side){return null}sync(track){var _a;if(this.dirty&2){var parent=this.dom,pos=null;for(let child of this.children){if(child.dirty){var next=pos?pos.nextSibling:parent.firstChild;child.dom||!next||(null===(_a=ContentView.get(next))||void 0===_a?0:_a.parent)||child.reuseDOM(next);child.sync(track);child.dirty=0}track&&track.node==
parent&&pos!=child.dom&&(track.written=!0);next=parent;var dom=child.dom;pos=pos?pos.nextSibling:next.firstChild;if(dom.parentNode==next)for(;pos!=dom;)pos=rm(pos);else next.insertBefore(dom,pos);pos=child.dom}(_a=pos?pos.nextSibling:parent.firstChild)&&track&&track.node==parent&&(track.written=!0);for(;_a;)_a=rm(_a)}else if(this.dirty&1)for(parent of this.children)parent.dirty&&(parent.sync(track),parent.dirty=0)}reuseDOM(_dom){return!1}localPosFromDOM(node,offset){if(node==this.dom)node=this.dom.childNodes[offset];
else{for(offset=0==maxOffset(node)?0:0==offset?-1:1;;){let parent=node.parentNode;if(parent==this.dom)break;0==offset&&parent.firstChild!=parent.lastChild&&(offset=node==parent.firstChild?-1:1);node=parent}node=0>offset?node:node.nextSibling}if(node==this.dom.firstChild)return 0;for(;node&&!ContentView.get(node);)node=node.nextSibling;if(!node)return this.length;for(let i=0,pos=0;;i++){offset=this.children[i];if(offset.dom==node)return pos;pos+=offset.length+offset.breakAfter}}domBoundsAround(from,
to,offset=0){let fromI=-1,fromStart=-1,toI=-1,toEnd=-1;for(let i=0,pos=offset,prevEnd=offset;i<this.children.length;i++){let child=this.children[i],end=pos+child.length;if(pos<from&&end>to)return child.domBoundsAround(from,to,pos);end>=from&&-1==fromI&&(fromI=i,fromStart=pos);if(pos>to&&child.dom.parentNode==this.dom){toI=i;toEnd=prevEnd;break}prevEnd=end;pos=end+child.breakAfter}return{from:fromStart,to:0>toEnd?offset+this.length:toEnd,startDOM:(fromI?this.children[fromI-1].dom.nextSibling:null)||
this.dom.firstChild,endDOM:toI<this.children.length&&0<=toI?this.children[toI].dom:null}}markDirty(andParent=!1){this.dirty&2||(this.dirty|=2,this.markParentsDirty(andParent))}markParentsDirty(childList){for(let parent=this.parent;parent;parent=parent.parent){childList&&(parent.dirty|=2);if(parent.dirty&1)break;parent.dirty|=1;childList=!1}}setParent(parent){this.parent!=parent&&(this.parent=parent,this.dirty&&this.markParentsDirty(!0))}setDOM(dom){this.dom=dom;dom.cmView=this}get rootView(){for(let v=
this;;){let parent=v.parent;if(!parent)return v;v=parent}}replaceChildren(from,to,children=none$3){this.markDirty();for(let i=from;i<to;i++){let child=this.children[i];child.parent==this&&(child.parent=null)}this.children.splice(from,to-from,...children);for(from=0;from<children.length;from++)children[from].setParent(this)}ignoreMutation(_rec){return!1}ignoreEvent(_event){return!1}childCursor(pos=this.length){return new ChildCursor(this.children,pos,this.children.length)}childPos(pos,bias=1){return this.childCursor().findPos(pos,
bias)}toString(){let name=this.constructor.name.replace("View","");return name+(this.children.length?"("+this.children.join()+")":this.length?"["+("Text"==name?this.text:this.length)+"]":"")+(this.breakAfter?"#":"")}static get(node){return node.cmView}}ContentView.prototype.breakAfter=0;class ChildCursor{constructor(children,pos,i){this.children=children;this.pos=pos;this.i=i;this.off=0}findPos(pos,bias=1){for(;;){if(pos>this.pos||pos==this.pos&&(0<bias||0==this.i||this.children[this.i-1].breakAfter))return this.off=
pos-this.pos,this;let next=this.children[--this.i];this.pos-=next.length+next.breakAfter}}}let [nav,doc$jscomp$0]="undefined"!=typeof navigator?[navigator,document]:[{userAgent:"",vendor:"",platform:""},{documentElement:{style:{}}}];const ie_edge=/Edge\/(\d+)/.exec(nav.userAgent),ie_upto10=/MSIE \d/.test(nav.userAgent),ie_11up=/Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(nav.userAgent),ie=!!(ie_upto10||ie_11up||ie_edge),gecko=!ie&&/gecko\/(\d+)/i.test(nav.userAgent),chrome=!ie&&/Chrome\/(\d+)/.exec(nav.userAgent),
webkit="webkitFontSmoothing"in doc$jscomp$0.documentElement.style,safari=!ie&&/Apple Computer/.test(nav.vendor);var browser={mac:/Mac/.test(nav.platform),ie,ie_version:ie_upto10?doc$jscomp$0.documentMode||6:ie_11up?+ie_11up[1]:ie_edge?+ie_edge[1]:0,gecko,gecko_version:gecko?+(/Firefox\/(\d+)/.exec(nav.userAgent)||[0,0])[1]:0,chrome:!!chrome,chrome_version:chrome?+chrome[1]:0,ios:safari&&(/Mobile\/\w+/.test(nav.userAgent)||2<nav.maxTouchPoints),android:/Android\b/.test(nav.userAgent),webkit,safari,
webkit_version:webkit?+(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent)||[0,0])[1]:0,tabSize:null!=doc$jscomp$0.documentElement.style.tabSize?"tab-size":"-moz-tab-size"};const none$2=[];class InlineView extends ContentView{become(_other){return!1}getSide(){return 0}}InlineView.prototype.children=none$2;class TextView extends InlineView{constructor(text){super();this.text=text}get length(){return this.text.length}createDOM(textDOM){this.setDOM(textDOM||document.createTextNode(this.text))}sync(track){this.dom||
this.createDOM();this.dom.nodeValue!=this.text&&(track&&track.node==this.dom&&(track.written=!0),this.dom.nodeValue=this.text)}reuseDOM(dom){if(3!=dom.nodeType)return!1;this.createDOM(dom);return!0}merge(from,to,source){if(source&&(!(source instanceof TextView)||256<this.length-(to-from)+source.length))return!1;this.text=this.text.slice(0,from)+(source?source.text:"")+this.text.slice(to);this.markDirty();return!0}slice(from){return new TextView(this.text.slice(from))}localPosFromDOM(node,offset){return node==
this.dom?offset:offset?this.text.length:0}domAtPos(pos){return new DOMPos(this.dom,pos)}domBoundsAround(_from,_to,offset){return{from:offset,to:offset+this.length,startDOM:this.dom,endDOM:this.dom.nextSibling}}coordsAt(pos,side){return textCoords(this.dom,pos,side)}}class MarkView extends InlineView{constructor(mark,children=[],length=0){super();this.mark=mark;this.children=children;this.length=length;for(let ch of children)ch.setParent(this)}createDOM(){let dom=document.createElement(this.mark.tagName);
this.mark.class&&(dom.className=this.mark.class);if(this.mark.attrs)for(let name in this.mark.attrs)dom.setAttribute(name,this.mark.attrs[name]);this.setDOM(dom)}sync(track){this.dom||this.createDOM();super.sync(track)}merge(from,to,source,openStart,openEnd){if(source&&(!(source instanceof MarkView&&source.mark.eq(this.mark))||from&&0>=openStart||to<this.length&&0>=openEnd))return!1;mergeInlineChildren(this,from,to,source?source.children:none$2,openStart-1,openEnd-1);this.markDirty();return!0}slice(from){var JSCompiler_temp_const=
this.mark,children=this.children;let result=[],off=0;for(let elt of children)children=off+elt.length,children>from&&result.push(off<from?elt.slice(from-off):elt),off=children;return new MarkView(JSCompiler_temp_const,result,this.length-from)}domAtPos(pos){return inlineDOMAtPos(this.dom,this.children,pos)}coordsAt(pos,side){return coordsInChildren(this,pos,side)}}class WidgetView extends InlineView{constructor(widget,length,side){super();this.widget=widget;this.length=length;this.side=side}static create(widget,
length,side){return new (widget.customView||WidgetView)(widget,length,side)}slice(from){return WidgetView.create(this.widget,this.length-from,this.side)}sync(){this.dom&&this.widget.updateDOM(this.dom)||(this.setDOM(this.widget.toDOM(this.editorView)),this.dom.contentEditable="false")}getSide(){return this.side}merge(from,to,source,openStart,openEnd){if(source&&(!(source instanceof WidgetView&&this.widget.compare(source.widget))||0<from&&0>=openStart||to<this.length&&0>=openEnd))return!1;this.length=
from+(source?source.length:0)+(this.length-to);return!0}become(other){return other.length==this.length&&other instanceof WidgetView&&other.side==this.side&&this.widget.constructor==other.widget.constructor?(this.widget.eq(other.widget)||this.markDirty(!0),this.widget=other.widget,!0):!1}ignoreMutation(){return!0}ignoreEvent(event){return this.widget.ignoreEvent(event)}get overrideDOMText(){if(0==this.length)return text$jscomp$0.Text.empty;for(var top=this;top.parent;)top=top.parent;top=(top=top.editorView)&&
top.state.doc;let start=this.posAtStart;return top?top.slice(start,start+this.length):text$jscomp$0.Text.empty}domAtPos(pos){return 0==pos?DOMPos.before(this.dom):DOMPos.after(this.dom,pos==this.length)}domBoundsAround(){return null}coordsAt(pos,side){let rects=this.dom.getClientRects(),rect;if(!rects.length)return Rect0;for(let i=0<pos?rects.length-1:0;!(rect=rects[i],0<pos?0==i:i==rects.length-1||rect.top<rect.bottom);i+=0<pos?-1:1);return 0==pos&&0<side||pos==this.length&&0>=side?rect:flattenRect(rect,
0==pos)}}class CompositionView extends WidgetView{domAtPos(pos){return new DOMPos(this.widget.text,pos)}sync(){this.dom||this.setDOM(this.widget.toDOM())}localPosFromDOM(node,offset){return offset?3==node.nodeType?Math.min(offset,this.length):this.length:0}ignoreMutation(){return!1}get overrideDOMText(){return null}coordsAt(pos,side){return textCoords(this.widget.text,pos,side)}}class WidgetType{eq(_widget){return!1}updateDOM(_dom){return!1}compare(other){return this==other||this.constructor==other.constructor&&
this.eq(other)}get estimatedHeight(){return-1}ignoreEvent(_event){return!0}get customView(){return null}}exports.BlockType=void 0;(function(BlockType){BlockType[BlockType.Text=0]="Text";BlockType[BlockType.WidgetBefore=1]="WidgetBefore";BlockType[BlockType.WidgetAfter=2]="WidgetAfter";BlockType[BlockType.WidgetRange=3]="WidgetRange"})(exports.BlockType||(exports.BlockType={}));class Decoration extends rangeset.RangeValue{constructor(startSide,endSide,widget,spec){super();this.startSide=startSide;
this.endSide=endSide;this.widget=widget;this.spec=spec}get heightRelevant(){return!1}static mark(spec){return new MarkDecoration(spec)}static widget(spec){let side=spec.side||0;spec.block&&(side+=(2E8+1)*(0<side?1:-1));return new PointDecoration(spec,side,side,!!spec.block,spec.widget||null,!1)}static replace(spec){let block=!!spec.block,{start,end}=getInclusive(spec);return new PointDecoration(spec,block?-2E8*(start?2:1):1E8*(start?-1:1),block?2E8*(end?2:1):1E8*(end?1:-1),block,spec.widget||null,
!0)}static line(spec){return new LineDecoration(spec)}static set(of,sort=!1){return rangeset.RangeSet.of(of,sort)}hasHeight(){return this.widget?-1<this.widget.estimatedHeight:!1}}Decoration.none=rangeset.RangeSet.empty;class MarkDecoration extends Decoration{constructor(spec){let {start,end}=getInclusive(spec);super(1E8*(start?-1:1),1E8*(end?1:-1),null,spec);this.tagName=spec.tagName||"span";this.class=spec.class||"";this.attrs=spec.attributes||null}eq(other){return this==other||other instanceof
...
!(last instanceof BlockWidgetView&&last.type==exports.BlockType.WidgetBefore)}getLine(){this.curLine||this.content.push(this.curLine=new LineView);return this.curLine}addWidget(view){this.curLine=null;this.content.push(view)}finish(){this.posCovered()||this.getLine()}wrapMarks(view,active){for(let mark of active)view=new MarkView(mark,[view],view.length);return view}buildText(length,active,openStart){for(;0<length;){if(this.textOff==this.text.length){let {value,lineBreak,done}=this.cursor.next(this.skip);
...
inputHandler=state$jscomp$0.Facet.define(),editable=state$jscomp$0.Facet.define({combine:values=>values.length?values[0]:!0});class PluginFieldProvider{constructor(field,get){this.field=field;this.get=get}}class PluginField{from(get){return new PluginFieldProvider(this,get)}static define(){return new PluginField}}PluginField.decorations=PluginField.define();PluginField.atomicRanges=PluginField.define();PluginField.scrollMargins=PluginField.define();let nextPluginID=0;const viewPlugin=state$jscomp$0.Facet.define();
class ViewPlugin{constructor(id,create,fields){this.id=id;this.create=create;this.fields=fields;this.extension=viewPlugin.of(this)}static define(create,spec){let {eventHandlers,provide,decorations}=spec||{};spec=[];if(provide)for(let provider of Array.isArray(provide)?provide:[provide])spec.push(provider);eventHandlers&&spec.push(domEventHandlers.from(value=>({plugin:value,handlers:eventHandlers})));decorations&&spec.push(PluginField.decorations.from(decorations));return new ViewPlugin(nextPluginID++,
create,spec)}static fromClass(cls,spec){return ViewPlugin.define(view=>new cls(view),spec)}}const domEventHandlers=PluginField.define();class PluginInstance{constructor(spec){this.spec=spec;this.value=this.mustUpdate=null}takeField(type,target){for(let {field,get}of this.spec.fields)field==type&&target.push(get(this.value))}update(view){if(!this.value)try{this.value=this.spec.create(view)}catch(e){return logException(view.state,e,"CodeMirror plugin crashed"),PluginInstance.dummy}else if(this.mustUpdate){view=
this.mustUpdate;this.mustUpdate=null;if(!this.value.update)return this;try{this.value.update(view)}catch(e){logException(view.state,e,"CodeMirror plugin crashed");if(this.value.destroy)try{this.value.destroy()}catch(_){}return PluginInstance.dummy}}return this}destroy(view){var _a;if(null===(_a=this.value)||void 0===_a?0:_a.destroy)try{this.value.destroy()}catch(e){logException(view.state,e,"CodeMirror plugin crashed")}}}PluginInstance.dummy=new PluginInstance(ViewPlugin.define(()=>({})));const editorAttributes=
state$jscomp$0.Facet.define({combine:values=>values.reduce((a,b)=>combineAttrs(b,a),{})}),contentAttributes=state$jscomp$0.Facet.define({combine:values=>values.reduce((a,b)=>combineAttrs(b,a),{})}),decorations=state$jscomp$0.Facet.define(),styleModule=state$jscomp$0.Facet.define();class ChangedRange{constructor(fromA,toA,fromB,toB){this.fromA=fromA;this.toA=toA;this.fromB=fromB;this.toB=toB}join(other){return new ChangedRange(Math.min(this.fromA,other.fromA),Math.max(this.toA,other.toA),Math.min(this.fromB,
other.fromB),Math.max(this.toB,other.toB))}addToSet(set){let i=set.length,me=this;for(;0<i;i--){let range=set[i-1];if(!(range.fromA>me.toA)){if(range.toA<me.fromA)break;me=me.join(range);set.splice(i-1,1)}}set.splice(i,0,me);return set}static extendWithRanges(diff,ranges){if(0==ranges.length)return diff;let result=[];for(let dI=0,rI=0,posA=0,posB=0;;dI++){let next=dI==diff.length?null:diff[dI],off=posA-posB,end=next?next.fromB:1E9;for(;rI<ranges.length&&ranges[rI]<end;){let to=ranges[rI+1],fromB=
Math.max(posB,ranges[rI]),toB=Math.min(end,to);fromB<=toB&&(new ChangedRange(fromB+off,toB+off,fromB,toB)).addToSet(result);if(to>end)break;else rI+=2}if(!next)return result;(new ChangedRange(next.fromA,next.toA,next.fromB,next.toB)).addToSet(result);posA=next.toA;posB=next.toB}}}class ViewUpdate{constructor(view,state$1,transactions=none){this.view=view;this.state=state$1;this.transactions=transactions;this.flags=0;this.startState=view.state;this.changes=state$jscomp$0.ChangeSet.empty(this.startState.doc.length);
for(let tr of transactions)this.changes=this.changes.compose(tr.changes);let changedRanges=[];this.changes.iterChangedRanges((fromA,toA,fromB,toB)=>changedRanges.push(new ChangedRange(fromA,toA,fromB,toB)));this.changedRanges=changedRanges;state$1=view.hasFocus;state$1!=view.inputState.notifiedFocused&&(view.inputState.notifiedFocused=state$1,this.flags|=1);this.docChanged&&(this.flags|=2)}get viewportChanged(){return 0<(this.flags&4)}get heightChanged(){return 0<(this.flags&2)}get geometryChanged(){return this.docChanged||
0<(this.flags&18)}get focusChanged(){return 0<(this.flags&1)}get docChanged(){return this.transactions.some(tr=>tr.docChanged)}get selectionSet(){return this.transactions.some(tr=>tr.selection)}get empty(){return 0==this.flags&&0==this.transactions.length}}class DocView extends ContentView{constructor(view){super();this.view=view;this.compositionDeco=Decoration.none;this.decorations=[];this.minWidthTo=this.minWidthFrom=this.minWidth=0;this.impreciseHead=this.impreciseAnchor=null;this.setDOM(view.contentDOM);
this.children=[new LineView];this.children[0].setParent(this);this.updateInner([new ChangedRange(0,0,0,view.state.doc.length)],this.updateDeco(),0)}get root(){return this.view.root}get editorView(){return this.view}get length(){return this.view.state.doc.length}update(update){let changedRanges=update.changedRanges;0<this.minWidth&&changedRanges.length&&(changedRanges.every(({fromA,toA})=>toA<this.minWidthFrom||fromA>this.minWidthTo)?(this.minWidthFrom=update.changes.mapPos(this.minWidthFrom,1),this.minWidthTo=
update.changes.mapPos(this.minWidthTo,1)):this.minWidth=0);0>this.view.inputState.composing?this.compositionDeco=Decoration.none:update.transactions.length&&(this.compositionDeco=computeCompositionDeco(this.view,update.changes));let forceSelection=(browser.ie||browser.chrome)&&!this.compositionDeco.size&&update&&update.state.doc.lines!=update.startState.doc.lines;var prevDeco=this.decorations;let deco=this.updateDeco();prevDeco=findChangedDeco(prevDeco,deco,update.changes);changedRanges=ChangedRange.extendWithRanges(changedRanges,
prevDeco);prevDeco=update.transactions.some(tr=>"pointerselection"==tr.annotation(state$jscomp$0.Transaction.userEvent));if(0==this.dirty&&0==changedRanges.length&&!(update.flags&12)&&update.state.selection.main.from>=this.view.viewport.from&&update.state.selection.main.to<=this.view.viewport.to)return this.updateSelection(forceSelection,prevDeco),!1;this.updateInner(changedRanges,deco,update.startState.doc.length,forceSelection,prevDeco);return!0}updateInner(changes,deco,oldLength,forceSelection=
!1,pointerSel=!1){this.updateChildren(changes,deco,oldLength);let {observer}=this.view;observer.ignore(()=>{this.dom.style.height=this.view.viewState.domHeight+"px";this.dom.style.minWidth=this.minWidth?this.minWidth+"px":"";let track=browser.chrome||browser.ios?{node:observer.selectionRange.focusNode,written:!1}:void 0;this.sync(track);this.dirty=0;track&&(track.written||observer.selectionRange.focusNode!=track.node)&&(forceSelection=!0);this.updateSelection(forceSelection,pointerSel);this.dom.style.height=
""})}updateChildren(changes,deco,oldLength){oldLength=this.childCursor(oldLength);for(let i=changes.length-1;;i--){let next=0<=i?changes[i]:null;if(!next)break;let {fromA,toA,fromB,toB}=next,{content,breakAtStart,openStart,openEnd}=ContentBuilder.build(this.view.state.doc,fromB,toB,deco),{i:toI,off:toOff}=oldLength.findPos(toA,1),{i:fromI,off:fromOff}=oldLength.findPos(fromA,-1);this.replaceRange(fromI,fromOff,toI,toOff,content,breakAtStart,openStart,openEnd)}}replaceRange(fromI,fromOff,toI,toOff,
content,breakAtStart,openStart,openEnd){let before=this.children[fromI],last=content.length?content[content.length-1]:null,breakAtEnd=last?last.breakAfter:breakAtStart;if(fromI!=toI||breakAtStart||breakAtEnd||!(2>content.length)||!before.merge(fromOff,toOff,content.length?last:null,0==fromOff,openStart,openEnd)){var after=this.children[toI];toOff<after.length||after.children.length&&0==after.children[after.children.length-1].length?(fromI==toI&&(after=after.split(toOff),toOff=0),!breakAtEnd&&last&&
after.merge(0,toOff,last,!0,0,openEnd)?content[content.length-1]=after:((toOff||after.children.length&&0==after.children[0].length)&&after.merge(0,toOff,null,!1,0,openEnd),content.push(after))):after.breakAfter&&(last?last.breakAfter=1:breakAtStart=1);toI++;before.breakAfter=breakAtStart;0<fromOff&&(!breakAtStart&&content.length&&before.merge(fromOff,before.length,content[0],!1,openStart,0)?before.breakAfter=content.shift().breakAfter:(fromOff<before.length||before.children.length&&0==before.children[before.children.length-
1].length)&&before.merge(fromOff,before.length,null,!1,openStart,0),fromI++);for(;fromI<toI&&content.length;)if(this.children[toI-1].match(content[content.length-1]))toI--,content.pop();else if(this.children[fromI].match(content[0]))fromI++,content.shift();else break;(fromI<toI||content.length)&&this.replaceChildren(fromI,toI,content)}}updateSelection(force=!1,fromPointer=!1){if(!(!fromPointer&&!this.mayControlSelection()||browser.ios&&this.view.inputState.rapidCompositionStart)){var main=this.view.state.selection.main,
anchor=this.domAtPos(main.anchor),head=main.empty?anchor:this.domAtPos(main.head);if(browser.gecko&&main.empty&&1==anchor.node.nodeType&&anchor.node.firstChild&&(0==anchor.offset||"false"==anchor.node.childNodes[anchor.offset-1].contentEditable)&&(anchor.offset==anchor.node.childNodes.length||"false"==anchor.node.childNodes[anchor.offset].contentEditable)){let dummy=document.createTextNode("");this.view.observer.ignore(()=>anchor.node.insertBefore(dummy,anchor.node.childNodes[anchor.offset]||null));
anchor=head=new DOMPos(dummy,0);force=!0}var domSel=this.view.observer.selectionRange;!force&&domSel.focusNode&&isEquivalentPosition(anchor.node,anchor.offset,domSel.anchorNode,domSel.anchorOffset)&&isEquivalentPosition(head.node,head.offset,domSel.focusNode,domSel.focusOffset)||(this.view.observer.ignore(()=>{let rawSel=getSelection(this.root);if(main.empty){if(browser.gecko){var JSCompiler_inline_result=anchor.node;var offset=anchor.offset;JSCompiler_inline_result=1!=JSCompiler_inline_result.nodeType?
0:(offset&&"false"==JSCompiler_inline_result.childNodes[offset-1].contentEditable?1:0)|(offset<JSCompiler_inline_result.childNodes.length&&"false"==JSCompiler_inline_result.childNodes[offset].contentEditable?2:0);JSCompiler_inline_result&&3!=JSCompiler_inline_result&&(offset=nearbyTextNode(anchor.node,anchor.offset,1==JSCompiler_inline_result?1:-1))&&(anchor=new DOMPos(offset,1==JSCompiler_inline_result?0:offset.nodeValue.length))}rawSel.collapse(anchor.node,anchor.offset);null!=main.bidiLevel&&null!=
domSel.cursorBidiLevel&&(domSel.cursorBidiLevel=main.bidiLevel)}else rawSel.extend?(rawSel.collapse(anchor.node,anchor.offset),rawSel.extend(head.node,head.offset)):(JSCompiler_inline_result=document.createRange(),main.anchor>main.head&&([anchor,head]=[head,anchor]),JSCompiler_inline_result.setEnd(head.node,head.offset),JSCompiler_inline_result.setStart(anchor.node,anchor.offset),rawSel.removeAllRanges(),rawSel.addRange(JSCompiler_inline_result))}),this.view.observer.setSelectionRange(anchor,head));
this.impreciseAnchor=anchor.precise?null:new DOMPos(domSel.anchorNode,domSel.anchorOffset);this.impreciseHead=head.precise?null:new DOMPos(domSel.focusNode,domSel.focusOffset)}}enforceCursorAssoc(){if(!this.view.composing){var cursor=this.view.state.selection.main,sel=getSelection(this.root);if(cursor.empty&&cursor.assoc&&sel.modify){var line=LineView.find(this,cursor.head);if(line){var lineStart=line.posAtStart;cursor.head!=lineStart&&cursor.head!=lineStart+line.length&&(line=this.coordsAt(cursor.head,
-1),lineStart=this.coordsAt(cursor.head,1),!line||!lineStart||line.bottom>lineStart.top||(line=this.domAtPos(cursor.head+cursor.assoc),sel.collapse(line.node,line.offset),sel.modify("move",0>cursor.assoc?"forward":"backward","lineboundary")))}}}}mayControlSelection(){return this.view.state.facet(editable)?this.root.activeElement==this.dom:hasSelection(this.dom,this.view.observer.selectionRange)}nearest(dom){for(;dom;){let domView=ContentView.get(dom);if(domView&&domView.rootView==this)return domView;
dom=dom.parentNode}return null}posFromDOM(node,offset){let view=this.nearest(node);if(!view)throw new RangeError("Trying to find position for a DOM position outside of the document");return view.localPosFromDOM(node,offset)+view.posAtStart}domAtPos(pos){let {i,off}=this.childCursor().findPos(pos,-1);for(;i<this.children.length-1;){pos=this.children[i];if(off<pos.length||pos instanceof LineView)break;i++;off=0}return this.children[i].domAtPos(off)}coordsAt(pos,side){for(let off=this.length,i=this.children.length-
1;;i--){let child=this.children[i],start=off-child.breakAfter-child.length;if(pos>start||pos==start&&child.type!=exports.BlockType.WidgetBefore&&child.type!=exports.BlockType.WidgetAfter&&(!i||2==side||this.children[i-1].breakAfter||this.children[i-1].type==exports.BlockType.WidgetBefore&&-2<side))return child.coordsAt(pos-start,side);off=start}}measureVisibleLineHeights(){let result=[],{from,to}=this.view.viewState.viewport,minWidth=Math.max(this.view.scrollDOM.clientWidth,this.minWidth)+1;for(let pos=
0,i=0;i<this.children.length;i++){let child=this.children[i],end=pos+child.length;if(end>to)break;if(pos>=from){result.push(child.dom.getBoundingClientRect().height);let width=child.dom.scrollWidth;width>minWidth&&(this.minWidth=minWidth=width,this.minWidthFrom=pos,this.minWidthTo=end)}pos=end+child.breakAfter}return result}measureTextSize(){for(let child of this.children)if(child instanceof LineView){let measure=child.measureTextSize();if(measure)return measure}let dummy=document.createElement("div"),
lineHeight,charWidth;dummy.className="cm-line";dummy.textContent="abc def ghi jkl mno pqr stu";this.view.observer.ignore(()=>{this.dom.appendChild(dummy);let rect=clientRectsFor(dummy.firstChild)[0];lineHeight=dummy.getBoundingClientRect().height;charWidth=rect?rect.width/27:7;dummy.remove()});return{lineHeight,charWidth}}childCursor(pos=this.length){let i=this.children.length;i&&(pos-=this.children[--i].length);return new ChildCursor(this.children,pos,i)}computeBlockGapDeco(){let deco=[],vs=this.view.viewState;
for(let pos=0,i=0;;i++){let next=i==vs.viewports.length?null:vs.viewports[i],end=next?next.from-1:this.length;if(end>pos){let height=vs.lineAt(end,0).bottom-vs.lineAt(pos,0).top;deco.push(Decoration.replace({widget:new BlockGapWidget(height),block:!0,inclusive:!0}).range(pos,end))}if(!next)break;pos=next.to+1}return Decoration.set(deco)}updateDeco(){return this.decorations=[...this.view.pluginField(PluginField.decorations),...this.view.state.facet(decorations),this.compositionDeco,this.computeBlockGapDeco(),
this.view.viewState.lineGapDeco]}scrollPosIntoView(pos,side){if(pos=this.coordsAt(pos,side)){var mLeft=0,mRight=0,mTop=0,mBottom=0;for(var margins of this.view.pluginField(PluginField.scrollMargins))if(margins){let {left,right,top,bottom}=margins;null!=left&&(mLeft=Math.max(mLeft,left));null!=right&&(mRight=Math.max(mRight,right));null!=top&&(mTop=Math.max(mTop,top));null!=bottom&&(mBottom=Math.max(mBottom,bottom))}side=this.dom;margins={left:pos.left-mLeft,top:pos.top-mTop,right:pos.right+mRight,
bottom:pos.bottom+mBottom};pos=side.ownerDocument.defaultView;for(side=side.parentNode;side;)if(1==side.nodeType){if(mLeft=side==document.body){mBottom=0;var JSCompiler_object_inline_right_3044=pos.innerWidth;var JSCompiler_object_inline_top_3045=0;var JSCompiler_object_inline_bottom_3046=pos.innerHeight}else{if(side.scrollHeight<=side.clientHeight&&side.scrollWidth<=side.clientWidth){side=side.parentNode;continue}mRight=side.getBoundingClientRect();mBottom=mRight.left;JSCompiler_object_inline_right_3044=
mRight.left+side.clientWidth;JSCompiler_object_inline_top_3045=mRight.top;JSCompiler_object_inline_bottom_3046=mRight.top+side.clientHeight}mTop=mRight=0;margins.top<JSCompiler_object_inline_top_3045?mTop=-(JSCompiler_object_inline_top_3045-margins.top+5):margins.bottom>JSCompiler_object_inline_bottom_3046&&(mTop=margins.bottom-JSCompiler_object_inline_bottom_3046+5);margins.left<mBottom?mRight=-(mBottom-margins.left+5):margins.right>JSCompiler_object_inline_right_3044&&(mRight=margins.right-JSCompiler_object_inline_right_3044+
5);if(mRight||mTop)mLeft?pos.scrollBy(mRight,mTop):(mTop&&(mBottom=side.scrollTop,side.scrollTop+=mTop,mTop=side.scrollTop-mBottom),mRight&&(mBottom=side.scrollLeft,side.scrollLeft+=mRight,mRight=side.scrollLeft-mBottom),margins={left:margins.left-mRight,top:margins.top-mTop,right:margins.right-mRight,bottom:margins.bottom-mTop});if(mLeft)break;side=side.assignedSlot||side.parentNode}else if(11==side.nodeType)side=side.host;else break}}}class BlockGapWidget extends WidgetType{constructor(height){super();
this.height=height}toDOM(){let elt=document.createElement("div");this.updateDOM(elt);return elt}eq(other){return other.height==this.height}updateDOM(elt){elt.style.height=this.height+"px";return!0}get estimatedHeight(){return this.height}}class CompositionWidget extends WidgetType{constructor(top,text){super();this.top=top;this.text=text}eq(other){return this.top==other.top&&this.text==other.text}toDOM(){return this.top}ignoreEvent(){return!1}get customView(){return CompositionView}}class DecorationComparator$1{constructor(){this.changes=
[]}compareRange(from,to){addRange(from,to,this.changes)}comparePoint(from,to){addRange(from,to,this.changes)}}exports.Direction=void 0;(function(Direction){Direction[Direction.LTR=0]="LTR";Direction[Direction.RTL=1]="RTL"})(exports.Direction||(exports.Direction={}));const LTR=exports.Direction.LTR,RTL=exports.Direction.RTL,LowTypes=dec("88888888888888888888888888888888888666888888787833333333337888888000000000000000000000000008888880000000000000000000000000088888888888888888888888888888888888887866668888088888663380888308888800000000000000000000000800000000000000000000000000000008"),
...
this.mouseSelection.update(update);update.transactions.length&&(this.lastKeyCode=this.lastSelectionTime=0)}destroy(){this.mouseSelection&&this.mouseSelection.destroy()}}const modifierCodes=[16,17,18,20,91,92,224,225];class MouseSelection{constructor(inputState,view,startEvent,style){this.inputState=inputState;this.view=view;this.style=style;this.lastEvent=startEvent;inputState=view.contentDOM.ownerDocument;inputState.addEventListener("mousemove",this.move=this.move.bind(this));inputState.addEventListener("mouseup",
this.up=this.up.bind(this));this.extend=startEvent.shiftKey;if(inputState=view.state.facet(state$jscomp$0.EditorState.allowMultipleSelections))inputState=view.state.facet(clickAddsSelectionRange),inputState=inputState.length?inputState[0](startEvent):browser.mac?startEvent.metaKey:startEvent.ctrlKey;this.multiple=inputState;inputState=view.state.facet(dragMovesSelection$1);this.dragMove=inputState.length?inputState[0](startEvent):browser.mac?!startEvent.altKey:!startEvent.ctrlKey;this.dragging=isInPrimarySelection(view,
startEvent)?null:!1;!1===this.dragging&&(startEvent.preventDefault(),this.select(startEvent))}move(event){if(0==event.buttons)return this.destroy();!1===this.dragging&&this.select(this.lastEvent=event)}up(event){null==this.dragging&&this.select(this.lastEvent);this.dragging||event.preventDefault();this.destroy()}destroy(){let doc=this.view.contentDOM.ownerDocument;doc.removeEventListener("mousemove",this.move);doc.removeEventListener("mouseup",this.up);this.inputState.mouseSelection=null}select(event){event=
this.style.get(event,this.extend,this.multiple);event.eq(this.view.state.selection)&&event.main.assoc==this.view.state.selection.main.assoc||this.view.dispatch({selection:event,annotations:state$jscomp$0.Transaction.userEvent.of("pointerselection"),scrollIntoView:!0})}update(update){update.docChanged&&this.dragging&&(this.dragging=this.dragging.map(update.changes));this.style.update(update)&&setTimeout(()=>this.select(this.lastEvent),20)}}const handlers=Object.create(null),brokenClipboardAPI=browser.ie&&
15>browser.ie_version||browser.ios&&604>browser.webkit_version;handlers.keydown=(view,event)=>{view.inputState.setSelectionOrigin("keyboardselection")};let lastTouch=0;handlers.touchstart=(view,e)=>{lastTouch=Date.now();view.inputState.setSelectionOrigin("pointerselection")};handlers.touchmove=view=>{view.inputState.setSelectionOrigin("pointerselection")};handlers.mousedown=(view,event)=>{view.observer.flush();if(!(lastTouch>Date.now()-2E3)){var style=null;for(let makeStyle of view.state.facet(mouseSelectionStyle))if(style=
makeStyle(view,event))break;style||0!=event.button||(style=basicMouseSelection(view,event));style&&(view.root.activeElement!=view.contentDOM&&view.observer.ignore(()=>focusPreventScroll(view.contentDOM)),view.inputState.startMouseSelection(view,event,style))}};const BadMouseDetail=browser.ie&&11>=browser.ie_version;let lastMouseDown=null,lastMouseDownCount=0,lastMouseDownTime=0;handlers.dragstart=(view,event)=>{let {selection:{main}}=view.state,{mouseSelection}=view.inputState;mouseSelection&&(mouseSelection.dragging=
...
new FileReader;reader.onerror=finishFile;reader.onload=()=>{/[\x00-\x08\x0e-\x1f]{2}/.test(reader.result)||(text[i]=reader.result);finishFile()};reader.readAsText(files[i])}}else dropText(view,event,event.dataTransfer.getData("Text"),!0)}};handlers.paste=(view,event)=>{if(view.state.facet(editable)){view.observer.flush();var data=brokenClipboardAPI?null:event.clipboardData;(data=data&&data.getData("text/plain"))?(doPaste(view,data),event.preventDefault()):capturePaste(view)}};let lastLinewiseCopy=
...
0;update<this.plugins.length;update++)this.plugins[update]=this.plugins[update].update(this)}measure(flush=!0){-1<this.measureScheduled&&cancelAnimationFrame(this.measureScheduled);this.measureScheduled=-1;flush&&this.observer.flush();flush=null;try{for(let i$jscomp$0=0;;i$jscomp$0++){this.updateState=1;let changed=this.viewState.measure(this.docView,0<i$jscomp$0),measuring=this.measureRequests;if(!changed&&!measuring.length&&null==this.viewState.scrollTo)break;this.measureRequests=[];if(5<i$jscomp$0){console.warn("Viewport failed to stabilize");
break}let measured=measuring.map(m=>{try{return m.read(this)}catch(e){return logException(this.state,e),BadMeasure}}),update=new ViewUpdate(this,this.state);update.flags|=changed;flush?flush.flags|=changed:flush=update;this.updateState=2;update.empty||(this.updatePlugins(update),this.inputState.update(update));this.updateAttrs();changed&&this.docView.update(update);for(let i=0;i<measuring.length;i++)if(measured[i]!=BadMeasure)try{measuring[i].write(measured[i],this)}catch(e){logException(this.state,
e)}this.viewState.scrollTo&&(this.docView.scrollPosIntoView(this.viewState.scrollTo.head,this.viewState.scrollTo.assoc),this.viewState.scrollTo=null);if(!(changed&4)&&0==this.measureRequests.length)break}}finally{this.updateState=0}this.measureScheduled=-1;if(flush&&!flush.empty)for(let listener of this.state.facet(updateListener))listener(flush)}get themeClasses(){return baseThemeID+" "+(this.state.facet(darkTheme)?baseDarkID:baseLightID)+" "+this.state.facet(theme)}updateAttrs(){var editorAttrs=
combineAttrs(this.state.facet(editorAttributes),{class:"cm-editor cm-wrap"+(this.hasFocus?" cm-focused ":" ")+this.themeClasses});updateAttrs(this.dom,this.editorAttrs,editorAttrs);this.editorAttrs=editorAttrs;editorAttrs=this.state.facet(contentAttributes);if(this.state.facet(editable)){if(null==_plainTextSupported){_plainTextSupported=!1;var JSCompiler_temp=document.createElement("div");try{JSCompiler_temp.contentEditable="plaintext-only",_plainTextSupported="plaintext-only"==JSCompiler_temp.contentEditable}catch(_){}}JSCompiler_temp=
_plainTextSupported?"plaintext-only":"true"}else JSCompiler_temp="false";editorAttrs=combineAttrs(editorAttrs,{spellcheck:"false",autocorrect:"off",autocapitalize:"off",contenteditable:JSCompiler_temp,class:"cm-content",style:`${browser.tabSize}: ${this.state.tabSize}`,role:"textbox","aria-multiline":"true"});updateAttrs(this.contentDOM,this.contentAttrs,editorAttrs);this.contentAttrs=editorAttrs}showAnnouncements(trs){let first=!0;for(let tr of trs)for(let effect of tr.effects)effect.is(EditorView.announce)&&
...
(this.measureScheduled=requestAnimationFrame(()=>this.measure()));if(request){if(null!=request.key)for(let i=0;i<this.measureRequests.length;i++)if(this.measureRequests[i].key===request.key){this.measureRequests[i]=request;return}this.measureRequests.push(request)}}pluginField(field){let result=[];for(let plugin of this.plugins)plugin.update(this).takeField(field,result);return result}plugin(plugin){for(let inst of this.plugins)if(inst.spec==plugin)return inst.update(this).value;return null}blockAtHeight(height,
docTop){this.readMeasured();return this.viewState.blockAtHeight(height,ensureTop(docTop,this.contentDOM))}visualLineAtHeight(height,docTop){this.readMeasured();return this.viewState.lineAtHeight(height,ensureTop(docTop,this.contentDOM))}viewportLines(f,docTop){let {from,to}=this.viewport;this.viewState.forEachLine(from,to,f,ensureTop(docTop,this.contentDOM))}visualLineAt(pos,docTop=0){return this.viewState.lineAt(pos,docTop)}get contentHeight(){return this.viewState.contentHeight}moveByChar(start,
...
let dir=forward?1:-1;if(JSCompiler_inline_result==(forward?this.state.doc.length:0))JSCompiler_inline_result=state$jscomp$0.EditorSelection.cursor(JSCompiler_inline_result);else{forward=start.goalColumn;var startY,rect=this.contentDOM.getBoundingClientRect();(startY=this.coordsAtPos(JSCompiler_inline_result))?(null==forward&&(forward=startY.left-rect.left),startY=0>dir?startY.top:startY.bottom):(startY=this.viewState.lineAt(JSCompiler_inline_result,this.dom.getBoundingClientRect().top),null==forward&&
(forward=Math.min(rect.right-rect.left,this.defaultCharacterWidth*(JSCompiler_inline_result-startY.from))),startY=0>dir?startY.top:startY.bottom);var resolvedGoal=rect.left+forward;distance=null!==distance&&void 0!==distance?distance:this.defaultLineHeight>>1;for(let extra=0;;extra+=10){let curY=startY+(distance+extra)*dir,pos=posAtCoords(this,{x:resolvedGoal,y:curY},!1,dir);if(curY<rect.top||curY>rect.bottom||(0>dir?pos<JSCompiler_inline_result:pos>JSCompiler_inline_result)){JSCompiler_inline_result=
state$jscomp$0.EditorSelection.cursor(pos,void 0,void 0,forward);break a}}}}return skipAtoms(this,start,JSCompiler_inline_result)}scrollPosIntoView(pos){this.viewState.scrollTo=state$jscomp$0.EditorSelection.cursor(pos);this.requestMeasure()}domAtPos(pos){return this.docView.domAtPos(pos)}posAtDOM(node,offset=0){return this.docView.posFromDOM(node,offset)}posAtCoords(coords,precise=!0){this.readMeasured();return posAtCoords(this,coords,precise)}coordsAtPos(pos,side=1){this.readMeasured();let rect=
...
filterTo:end,filter:()=>!1,add:ranges})}}return deco}}const UnicodeRegexpSupport=null!=/x/.unicode?"gu":"g",Specials=RegExp("[\x00-\b\n--­؜​‎‏  ﻿￹-￼]",UnicodeRegexpSupport),Names={0:"null",7:"bell",8:"backspace",10:"newline",11:"vertical tab",13:"carriage return",27:"escape",8203:"zero width space",8204:"zero width non-joiner",8205:"zero width joiner",8206:"left-to-right mark",8207:"right-to-left mark",8232:"line separator",8233:"paragraph separator",65279:"zero width no-break space",65532:"object replacement"};
let _supportsTabSize=null;const specialCharConfig=state$jscomp$0.Facet.define({combine(configs){configs=state$jscomp$0.combineConfig(configs,{render:null,specialChars:Specials,addSpecialChars:null});var _a;if(null==_supportsTabSize&&"undefined"!=typeof document&&document.body){let styles=document.body.style;_supportsTabSize=null!=(null!==(_a=styles.tabSize)&&void 0!==_a?_a:styles.MozTabSize)}if(configs.replaceTabs=!_supportsTabSize)configs.specialChars=new RegExp("\t|"+configs.specialChars.source,
UnicodeRegexpSupport);configs.addSpecialChars&&(configs.specialChars=new RegExp(configs.specialChars.source+"|"+configs.addSpecialChars.source,UnicodeRegexpSupport));return configs}});let _plugin=null;class SpecialCharWidget extends WidgetType{constructor(options,code){super();this.options=options;this.code=code}eq(other){return other.code==this.code}toDOM(view){var JSCompiler_inline_result=this.code;JSCompiler_inline_result=32<=JSCompiler_inline_result?"•":10==JSCompiler_inline_result?"␤":String.fromCharCode(9216+
JSCompiler_inline_result);view=view.state.phrase("Control character")+" "+(Names[this.code]||"0x"+this.code.toString(16));var custom=this.options.render&&this.options.render(this.code,view,JSCompiler_inline_result);if(custom)return custom;custom=document.createElement("span");custom.textContent=JSCompiler_inline_result;custom.title=view;custom.setAttribute("aria-label",view);custom.className="cm-specialChar";return custom}ignoreEvent(){return!1}}class TabWidget extends WidgetType{constructor(width){super();
