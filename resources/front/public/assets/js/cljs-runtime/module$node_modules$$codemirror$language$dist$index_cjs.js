...
-1;this.work=this.work.bind(this);this.scheduleWork()}update(update){let cx=this.view.state.field(Language.state).context;update.viewportChanged&&(cx.updateViewport(update.view.viewport)&&cx.reset(),this.view.viewport.to>cx.tree.length&&this.scheduleWork());update.docChanged&&(this.view.hasFocus&&(this.chunkBudget+=50),this.scheduleWork());this.checkAsyncSchedule(cx)}scheduleWork(){if(!(-1<this.working)){var {state}=this.view,field=state.field(Language.state),frags=field.context.fragments;field.tree.length>=
state.doc.length&&frags.length&&0==frags[0].from&&frags[0].to>=state.doc.length||(this.working=requestIdle(this.work,{timeout:500}))}}work(deadline){this.working=-1;let now=Date.now();this.chunkEnd<now&&(0>this.chunkEnd||this.view.hasFocus)&&(this.chunkEnd=now+3E4,this.chunkBudget=3E3);if(!(0>=this.chunkBudget)){var {state,viewport:{to:vpTo}}=this.view,field=state.field(Language.state);if(!(field.tree.length>=vpTo+1E6)){deadline=Math.min(this.chunkBudget,deadline?Math.max(25,deadline.timeRemaining()):
100);deadline=field.context.work(deadline,vpTo+1E6);this.chunkBudget-=Date.now()-now;if(deadline||0>=this.chunkBudget||field.context.movedPast(vpTo))field.context.takeTree(),this.view.dispatch({effects:Language.setState.of(new LanguageState(field.context))});!deadline&&0<this.chunkBudget&&this.scheduleWork();this.checkAsyncSchedule(field.context)}}}checkAsyncSchedule(cx){cx.scheduleOn&&(cx.scheduleOn.then(()=>this.scheduleWork()),cx.scheduleOn=null)}destroy(){0<=this.working&&cancelIdle(this.working)}},
{eventHandlers:{focus(){this.scheduleWork()}}});const language=state$jscomp$0.Facet.define({combine(languages){return languages.length?languages[0]:null},enables:[Language.state,global]});class LanguageSupport{constructor(language,support=[]){this.language=language;this.support=support;this.extension=[language,support]}}class LanguageDescription{constructor(name,alias,extensions,filename,loadFunc){this.name=name;this.alias=alias;this.extensions=extensions;this.filename=filename;this.loadFunc=loadFunc;
this.support=void 0;this.loading=null}load(){return this.loading||(this.loading=this.loadFunc().then(support=>this.support=support,err=>{this.loading=null;throw err;}))}static of(spec){return new LanguageDescription(spec.name,(spec.alias||[]).concat(spec.name).map(s=>s.toLowerCase()),spec.extensions||[],spec.filename,spec.load)}static matchFilename(descs,filename){for(let d of descs)if(d.filename&&d.filename.test(filename))return d;if(filename=/\.([^.]+)$/.exec(filename))for(let d of descs)if(-1<
...
JSON.stringify(values[0]));return values[0]}});class IndentContext{constructor(state,options={}){this.state=state;this.options=options;this.unit=getIndentUnit(state)}textAfterPos(pos){var _a,_b;let sim=null===(_a=this.options)||void 0===_a?void 0:_a.simulateBreak;return pos==sim&&(null===(_b=this.options)||void 0===_b?0:_b.simulateDoubleBreak)?"":this.state.sliceDoc(pos,Math.min(pos+100,null!=sim&&sim>pos?sim:1E9,this.state.doc.lineAt(pos).to))}column(pos){var _a;let line=this.state.doc.lineAt(pos),
text=line.text.slice(0,pos-line.from);pos=this.countColumn(text,pos-line.from);let override=(null===(_a=this.options)||void 0===_a?0:_a.overrideIndentation)?this.options.overrideIndentation(line.from):-1;-1<override&&(pos+=override-this.countColumn(text,text.search(/\S/)));return pos}countColumn(line,pos){return text$jscomp$0.countColumn(0>pos?line:line.slice(0,pos),0,this.state.tabSize)}lineIndent(line){var _a;let override=null===(_a=this.options)||void 0===_a?void 0:_a.overrideIndentation;return override&&
(_a=override(line.from),-1<_a)?_a:this.countColumn(line.text,line.text.search(/\S/))}}const indentNodeProp=new lezerTree.NodeProp;class TreeIndentContext extends IndentContext{constructor(base,pos,node){super(base.state,base.options);this.base=base;this.pos=pos;this.node=node}get textAfter(){return this.textAfterPos(this.pos)}get baseIndent(){let line=this.state.doc.lineAt(this.node.from);for(;;){let atBreak=this.node.resolve(line.from);for(;atBreak.parent&&atBreak.parent.from==atBreak.from;)atBreak=
atBreak.parent;var JSCompiler_inline_result;a:{for(JSCompiler_inline_result=this.node;JSCompiler_inline_result;JSCompiler_inline_result=JSCompiler_inline_result.parent)if(atBreak==JSCompiler_inline_result){JSCompiler_inline_result=!0;break a}JSCompiler_inline_result=!1}if(JSCompiler_inline_result)break;line=this.state.doc.lineAt(atBreak.from)}return this.lineIndent(line)}continue(){let parent=this.node.parent;return parent?indentFrom(parent,this.pos,this.base):0}}const foldService=state$jscomp$0.Facet.define(),
foldNodeProp=new lezerTree.NodeProp;exports.EditorParseContext=EditorParseContext;exports.IndentContext=IndentContext;exports.Language=Language;exports.LanguageDescription=LanguageDescription;exports.LanguageSupport=LanguageSupport;exports.LezerLanguage=LezerLanguage;exports.TreeIndentContext=TreeIndentContext;exports.continuedIndent=function({except,units=1}={}){return context=>{let matchExcept=except&&except.test(context.textAfter);return context.baseIndent+(matchExcept?0:units*context.unit)}};
exports.defineLanguageFacet=defineLanguageFacet;exports.delimitedIndent=function({closing,align=!0,units=1}){return context=>delimitedStrategy(context,align,units,closing)};exports.ensureSyntaxTree=function(state,upto,timeout=50){var _a;return(state=null===(_a=state.field(Language.state,!1))||void 0===_a?void 0:_a.context)?state.tree.length>=upto||state.work(timeout,upto)?state.tree:null:null};exports.flatIndent=context=>context.baseIndent;exports.foldInside=function(node){let first=node.firstChild,
last=node.lastChild;return first&&first.to<last.from?{from:first.to,to:last.type.isError?node.to:last.from}:null};exports.foldNodeProp=foldNodeProp;exports.foldService=foldService;exports.foldable=function(state,lineStart,lineEnd){for(var service of state.facet(foldService)){var result=service(state,lineStart,lineEnd);if(result)return result}service=syntaxTree(state);if(0==service.length)state=null;else{result=service.resolve(lineEnd);for(service=null;result;result=result.parent)if(!(result.to<=lineEnd||
result.from>lineEnd)){if(service&&result.from<lineStart)break;var prop=result.type.prop(foldNodeProp);prop&&(prop=prop(result,state))&&prop.from<=lineEnd&&prop.from>=lineStart&&prop.to>lineEnd&&(service=prop)}state=service}return state};exports.getIndentUnit=getIndentUnit;exports.getIndentation=getIndentation;exports.indentNodeProp=indentNodeProp;exports.indentOnInput=function(){return state$jscomp$0.EditorState.transactionFilter.of(tr=>{if(!tr.docChanged||"input"!=tr.annotation(state$jscomp$0.Transaction.userEvent))return tr;
