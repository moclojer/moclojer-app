...
right){return Object.entries(right).reduce((result,[key,rightValue])=>{const leftValue=result[key];if(Array.isArray(leftValue)&&Array.isArray(rightValue))return result[key]=leftValue.concat(rightValue),result;if(isObject(leftValue)&&isObject(rightValue))return result[key]=mergeRight(leftValue,rightValue),result;result[key]=rightValue;return result},Object.assign({},left))}function formatMessage(message,...positionals){return`${"[MSW]"} ${(0,import_outvariant.format)(message,...positionals)}`}function getWorkerByRegistration(registration,
absoluteWorkerUrl,findWorker){return[registration.active,registration.installing,registration.waiting].filter(state=>null!=state).find(worker2=>findWorker(worker2.scriptURL,absoluteWorkerUrl))||null}function printStartMessage(args={}){args.quiet||(console.groupCollapsed(`%c${devUtils.formatMessage(args.message||"Mocking enabled.")}`,"color:orangered;font-weight:bold;"),console.log("%cDocumentation: %chttps://mswjs.io/docs","font-weight:bold","font-weight:normal"),console.log("Found an issue? https://github.com/mswjs/msw/issues"),
args.workerUrl&&console.log("Worker script URL:",args.workerUrl),args.workerScope&&console.log("Worker scope:",args.workerScope),console.groupEnd())}async function enableMocking(context,options){var _a,_b;context.workerChannel.send("MOCK_ACTIVATE");await context.events.once("MOCKING_ENABLED");context.isMockingEnabled?devUtils.warn('Found a redundant "worker.start()" call. Note that starting the worker while mocking is already enabled will have no effect. Consider removing this "worker.start()" call.'):
(context.isMockingEnabled=!0,printStartMessage({quiet:options.quiet,workerScope:null==(_a=context.registration)?void 0:_a.scope,workerUrl:null==(_b=context.worker)?void 0:_b.scriptURL}))}function getRequestCookies(request){if("undefined"===typeof document||"undefined"===typeof location)return{};switch(request.credentials){case "same-origin":return location.origin===request.url.origin?cookieUtils2.parse(document.cookie):{};case "include":return cookieUtils2.parse(document.cookie);default:return{}}}
function parseContentHeaders(headersString){var _a,_b,headers=(0,import_headers_polyfill3.stringToHeaders)(headersString);headersString=headers.get("content-type")||"text/plain";headers=headers.get("content-disposition");if(!headers)throw Error('"Content-Disposition" header is required.');const directives=headers.split(";").reduce((acc,chunk)=>{const [name2,...rest2]=chunk.trim().split("\x3d");acc[name2]=rest2.join("\x3d");return acc},{});headers=null==(_a=directives.name)?void 0:_a.slice(1,-1);_a=
null==(_b=directives.filename)?void 0:_b.slice(1,-1);return{name:headers,filename:_a,contentType:headersString}}function parseMultipartData(data2,headers){if(headers=null==headers?void 0:headers.get("content-type"))if([,...headers]=headers.split(/; */),headers=headers.filter(d=>d.startsWith("boundary\x3d")).map(s=>s.replace(/^boundary=/,""))[0])if(data2=data2.split(new RegExp(`--+${headers}`)).filter(chunk=>chunk.startsWith("\r\n")&&chunk.endsWith("\r\n")).map(chunk=>chunk.trimStart().replace(/\r\n$/,
"")),data2.length){headers={};try{for(const field2 of data2){const [contentHeaders,...rest2]=field2.split("\r\n\r\n"),contentBody=rest2.join("\r\n\r\n"),{contentType:contentType2,filename,name}=parseContentHeaders(contentHeaders),value=void 0===filename?contentBody:new File([contentBody],filename,{type:contentType2}),parsedValue=headers[name];void 0===parsedValue?headers[name]=value:Array.isArray(parsedValue)?headers[name]=[...parsedValue,value]:headers[name]=[parsedValue,value]}return headers}catch(error2){}}}
function parseBody(body2,headers){var _a;if(!body2)return body2;const contentType=(null==(_a=null==headers?void 0:headers.get("content-type"))?void 0:_a.toLowerCase())||"";return contentType.startsWith("multipart/form-data")&&"object"!==typeof body2?parseMultipartData(body2.toString(),headers)||body2:contentType.includes("json")&&"object"!==typeof body2?jsonParse(body2.toString())||body2:body2}function isStringEqual(actual,expected){return actual.toLowerCase()===expected.toLowerCase()}function parseWorkerRequest(rawRequest){const url=
new URL(rawRequest.url),headers=new import_headers_polyfill5.Headers(rawRequest.headers);return new MockedRequest(url,{...rawRequest,body:(0,import_interceptors2.encodeBuffer)(rawRequest.body||""),headers})}function parseDocumentNode(node){var _a;node=node.definitions.find(def=>"OperationDefinition"===def.kind);return{operationType:null==node?void 0:node.operation,operationName:null==(_a=null==node?void 0:node.name)?void 0:_a.value}}function getGraphQLInput(request){var _a,_b;switch(request.method){case "GET":var query$jscomp$0=
request.url.searchParams.get("query"),variables$jscomp$0=request.url.searchParams.get("variables")||"";return{query:query$jscomp$0,variables:jsonParse(variables$jscomp$0)};case "POST":if(null==(_a=request.body)?0:_a.query){const {query,variables}=request.body;return{query,variables}}if(null==(_b=request.body)?0:_b.operations){const {operations,map,...files}=request.body;request=jsonParse(operations)||{};if(!request.query)return null;_a=jsonParse(map||"")||{};if(request.variables){var map$jscomp$0=
_a;_a=files;_b={variables:request.variables};for(const [key,pathArray]of Object.entries(map$jscomp$0)){if(!(key in _a))throw Error(`Given files do not have a key '${key}' .`);for(query$jscomp$0 of pathArray){const [lastPath,...reversedPaths]=query$jscomp$0.split(".").reverse();map$jscomp$0=reversedPaths.reverse();let target=_b;for(variables$jscomp$0 of map$jscomp$0){if(!(variables$jscomp$0 in target))throw Error(`Property '${map$jscomp$0}' is not in operations.`);target=target[variables$jscomp$0]}target[lastPath]=
_a[key]}}query$jscomp$0=_b.variables}else query$jscomp$0={};return{query:request.query,variables:query$jscomp$0}}default:return null}}function parseGraphQLRequest(request){var input=getGraphQLInput(request);if(input&&input.query){var {query,variables}=input;try{const ast=(0,import_graphql.parse)(query);var JSCompiler_inline_result=parseDocumentNode(ast)}catch(error2){JSCompiler_inline_result=error2}input=JSCompiler_inline_result;if(input instanceof Error)throw JSCompiler_inline_result=getPublicUrlFromRequest(request),
Error(devUtils.formatMessage('Failed to intercept a GraphQL request to "%s %s": cannot parse query. See the error message from the parser below.\n\n%s',request.method,JSCompiler_inline_result,input.message));return{operationType:input.operationType,operationName:input.operationName,variables}}}function getStatusCodeColor(status2){return 300>status2?"#69AB32":400>status2?"#F0BB4B":"#E95F5D"}function getTimestamp(){const now=new Date;return[now.getHours(),now.getMinutes(),now.getSeconds()].map(String).map(chunk=>
chunk.slice(0,2)).map(chunk=>chunk.padStart(2,"0")).join(":")}function prepareResponse(res){const responseHeaders=(0,import_headers_polyfill6.objectToHeaders)(res.headers);return{...res,body:parseBody(res.body,responseHeaders)}}function cleanUrl(path){return path.replace(REDUNDANT_CHARACTERS_EXP,"")}function coercePath(path){return path.replace(/([:a-zA-Z_-]*)(\*{1,2})+/g,(_,parameterName,wildcard)=>parameterName?parameterName.startsWith(":")?`${parameterName}${wildcard}`:`${parameterName}${"(.*)"}`:
...
"error")}):applyStrategy(strategy)}async function handleRequest(request,handlers,options,emitter,handleRequestOptions){var _a,_b,_c,_d,_e,_f;emitter.emit("request:start",request);if("true"===request.headers.get("x-msw-bypass"))emitter.emit("request:end",request),null==(_a=null==handleRequestOptions?void 0:handleRequestOptions.onPassthroughResponse)?void 0:_a.call(handleRequestOptions,request);else{var [lookupError,lookupResult]=await (0,import_until2.until)(()=>getResponse(request,handlers,null==
handleRequestOptions?void 0:handleRequestOptions.resolutionContext));if(lookupError)throw emitter.emit("unhandledException",lookupError,request),lookupError;var {handler,response:response2}=lookupResult;if(handler)if(response2)if(response2.passthrough)emitter.emit("request:end",request),null==(_d=null==handleRequestOptions?void 0:handleRequestOptions.onPassthroughResponse)?void 0:_d.call(handleRequestOptions,request);else return import_cookies2.store.add({...request,url:request.url.toString()},response2),
import_cookies2.store.persist(),emitter.emit("request:match",request),options=(null==(_e=null==handleRequestOptions?void 0:handleRequestOptions.transformResponse)?void 0:_e.call(handleRequestOptions,response2))||response2,null==(_f=null==handleRequestOptions?void 0:handleRequestOptions.onMockedResponse)?void 0:_f.call(handleRequestOptions,options,lookupResult),emitter.emit("request:end",request),options;else devUtils.warn("Expected response resolver to return a mocked response Object, but got %s. The original response is going to be used instead.\n\n  • %s\n    %s",
...
({payload:context}=await context.events.once("INTEGRITY_CHECK_RESPONSE"));if("3d6b9f06410d179a7f7404d4bf4c3c70"!==context)throw Error(`Currently active Service Worker (${context}) is behind the latest published one (${"3d6b9f06410d179a7f7404d4bf4c3c70"}).`);return serviceWorker}function deferNetworkRequestsUntil(predicatePromise){const originalXhrSend=window.XMLHttpRequest.prototype.send;window.XMLHttpRequest.prototype.send=function(...args){(0,import_until3.until)(()=>predicatePromise).then(()=>
...
- Set the "Service-Worker-Allowed" response header to allow out-of-scope workers.`)}function printStopMessage(args={}){args.quiet||console.log(`%c${devUtils.formatMessage("Mocking disabled.")}`,"color:orangered;font-weight:bold;")}function createResponseFromIsomorphicResponse(response2){return{...response2,ok:200<=response2.status&&300>response2.status,url:"",type:"default",status:response2.status,statusText:response2.statusText,headers:response2.headers,body:new ReadableStream,redirected:null!=response2.headers.get("Location"),
async text(){return response2.body||""},async json(){return JSON.parse(response2.body||"")},async arrayBuffer(){return(0,import_interceptors3.encodeBuffer)(response2.body||"")},bodyUsed:!1,formData:noop,blob:noop,clone:noop}}function createFallbackRequestListener(context,options){const interceptor=new import_interceptors4.BatchInterceptor({name:"fallback",interceptors:[new import_fetch3.FetchInterceptor,new import_XMLHttpRequest.XMLHttpRequestInterceptor]});interceptor.on("request",async request=>
{var mockedRequest=new MockedRequest(request.url,{...request,body:await request.arrayBuffer()});(mockedRequest=await handleRequest(mockedRequest,context.requestHandlers,options,context.emitter,{transformResponse(response3){return{status:response3.status,statusText:response3.statusText,headers:response3.headers.all(),body:response3.body,delay:response3.delay}},onMockedResponse(_,{handler,publicRequest,parsedRequest}){if(!options.quiet)context.emitter.once("response:mocked",response3=>{handler.log(publicRequest,
serializeResponse(response3),parsedRequest)})}}))&&request.respondWith(mockedRequest)});interceptor.on("response",(request,response2)=>{if(request.id){var browserResponse=createResponseFromIsomorphicResponse(response2);"msw"===response2.headers.get("x-powered-by")?context.emitter.emit("response:mocked",browserResponse,request.id):context.emitter.emit("response:bypass",browserResponse,request.id)}});interceptor.apply();return interceptor}function createFallbackStart(context){return async function(options){context.fallbackInterceptor=
...
delayTime=durationOrMode}res.delay=delayTime;return res},errors=errorsList=>res=>{if(null==errorsList)return res;var prevBody=jsonParse(res.body)||{};prevBody=mergeRight(prevBody,{errors:errorsList});return json(prevBody)(res)},import_is_node_process2=require("module$node_modules$is_node_process$lib$index"),import_headers_polyfill2=require("module$node_modules$headers_polyfill$lib$index"),useFetch=(0,import_is_node_process2.isNodeProcess)()?(input,init)=>Promise.resolve().then(()=>__toESM(require("module$node_modules$node_fetch$browser"))).then(({default:nodeFetch})=>
nodeFetch(input,init)):globalThis.fetch,augmentRequestInit=requestInit=>{const headers=new import_headers_polyfill2.Headers(requestInit.headers);headers.set("x-msw-bypass","true");return{...requestInit,headers:headers.all()}},fetch=(input,requestInit={})=>{if("string"===typeof input)return useFetch(input,augmentRequestInit(requestInit));{const {body:body2,method}=input;requestInit={...input,body:void 0};["GET","HEAD"].includes(method)||(requestInit.body="object"===typeof body2||"number"===typeof body2||
"boolean"===typeof body2?JSON.stringify(body2):body2)}requestInit=augmentRequestInit(requestInit);return useFetch(input.url.href,requestInit)},text=body2=>res=>{res.headers.set("Content-Type","text/plain");res.body=body2;return res},xml=body2=>res=>{res.headers.set("Content-Type","text/xml");res.body=body2;return res},import_outvariant2=require("module$node_modules$outvariant$lib$index"),import_outvariant=require("module$node_modules$outvariant$lib$index"),devUtils={formatMessage,warn:function(message,
...positionals){console.warn(formatMessage(message,...positionals))},error:function(message,...positionals){console.error(formatMessage(message,...positionals))}},import_outvariant5=require("module$node_modules$outvariant$lib$index"),import_is_node_process3=require("module$node_modules$is_node_process$lib$index"),import_until4=require("module$node_modules$$open_draft$until$lib$index"),import_until=require("module$node_modules$$open_draft$until$lib$index"),getWorkerInstance=async(url,options={},findWorker)=>
{const absoluteWorkerUrl=(new URL(url,location.href)).href;var mockRegistrations=await navigator.serviceWorker.getRegistrations().then(registrations=>registrations.filter(registration=>getWorkerByRegistration(registration,absoluteWorkerUrl,findWorker)));!navigator.serviceWorker.controller&&0<mockRegistrations.length&&location.reload();const [existingRegistration]=mockRegistrations;if(existingRegistration)return existingRegistration.update().then(()=>[getWorkerByRegistration(existingRegistration,absoluteWorkerUrl,
findWorker),existingRegistration]);const [error2,instance]=await (0,import_until.until)(async()=>{const registration=await navigator.serviceWorker.register(url,options);return[getWorkerByRegistration(registration,absoluteWorkerUrl,findWorker),registration]});if(error2){if(error2.message.includes("(404)"))throw mockRegistrations=new URL((null==options?void 0:options.scope)||"/",location.href),Error(devUtils.formatMessage(`Failed to register a Service Worker for scope ('${mockRegistrations.href}') with script ('${absoluteWorkerUrl}'): Service Worker script does not exist at the given path.
...
{};import_cookies.store.hydrate();const cookiesFromStore=Array.from(null==(_a=import_cookies.store.get({...this,url:this.url.href}))?void 0:_a.entries()).reduce((cookies,[name,{value}])=>Object.assign(cookies,{[name.trim()]:value}),{});_a={...getRequestCookies(this),...cookiesFromStore};for(const [name,value]of Object.entries(_a))this.headers.append("cookie",`${name}=${value}`);return{..._a,...requestCookiesString}}},import_until2=require("module$node_modules$$open_draft$until$lib$index"),getResponse=
async(request,handlers,resolutionContext)=>{handlers=handlers.filter(handler=>handler.test(request,resolutionContext));return 0===handlers.length?{handler:void 0,response:void 0}:(handlers=await handlers.reduce(async(executionResult,handler)=>{const previousResults=await executionResult;if(null==previousResults?0:previousResults.response)return executionResult;executionResult=await handler.run(request,resolutionContext);if(null===executionResult||executionResult.handler.shouldSkip)return null;if(!executionResult.response)return{request:executionResult.request,
handler:executionResult.handler,response:void 0,parsedResult:executionResult.parsedResult};executionResult.response.once&&handler.markAsSkipped(!0);return executionResult},Promise.resolve(null)))?{handler:handlers.handler,publicRequest:handlers.request,parsedRequest:handlers.parsedResult,response:handlers.response}:{handler:void 0,response:void 0}},import_js_levenshtein=__toESM(require("module$node_modules$js_levenshtein$index")),import_graphql=require("module$node_modules$graphql$index"),getPublicUrlFromRequest=
request=>request.referrer.startsWith(request.url.origin)?request.url.pathname:(new URL(request.url.pathname,`${request.url.protocol}//${request.url.host}`)).href,import_headers_polyfill6=require("module$node_modules$headers_polyfill$lib$index"),import_path_to_regexp=require("module$node_modules$path_to_regexp$dist$index"),import_getCleanUrl=require("module$node_modules$$mswjs$interceptors$lib$utils$getCleanUrl"),REDUNDANT_CHARACTERS_EXP=/[\?|#].*$/g;require("module$node_modules$headers_polyfill$lib$index");
var import_headers_polyfill7=require("module$node_modules$headers_polyfill$lib$index"),defaultResponse={status:200,statusText:"OK",body:null,delay:0,once:!1,passthrough:!1},defaultResponseTransformers=[],response=Object.assign(createResponseComposition(),{once:createResponseComposition({once:!0}),networkError(message){throw new NetworkError(message);}}),SOURCE_FRAME=/[\/\\]msw[\/\\]src[\/\\](.+)/,BUILD_FRAME=/(node_modules)?[\/\\]lib[\/\\](umd|esm|iief|cjs)[\/\\]|^[^\/\\]*$/,defaultContext={status,
set,delay,fetch},RequestHandler=class{constructor(options){this.shouldSkip=!1;this.ctx=options.ctx||defaultContext;this.resolver=options.resolver;const callFrame=getCallFrame(Error());this.info={...options.info,callFrame}}parse(_request,_resolutionContext){return null}test(request,resolutionContext){return this.predicate(request,this.parse(request,resolutionContext),resolutionContext)}getPublicRequest(request,_parsedResult){return request}markAsSkipped(shouldSkip=!0){this.shouldSkip=shouldSkip}async run(request,
resolutionContext){if(this.shouldSkip)return null;const parsedResult=this.parse(request,resolutionContext);if(!this.predicate(request,parsedResult,resolutionContext))return null;request=this.getPublicRequest(request,parsedResult);resolutionContext=await this.wrapResolver(this.resolver)(request,response,this.ctx);return this.createExecutionResult(parsedResult,request,resolutionContext)}wrapResolver(resolver){return async(req,res,ctx)=>{req=this.resolverGenerator||await resolver(req,res,ctx);res=req?
"function"==typeof req[Symbol.iterator]:!1;if(res){const {value,done}=req[Symbol.iterator]().next();res=await value;if(!res&&done)return this.resolverGeneratorResult;this.resolverGenerator||(this.resolverGenerator=req);return this.resolverGeneratorResult=res}return req}}createExecutionResult(parsedResult,request,response2){return{handler:this,parsedResult:parsedResult||null,request,response:response2||null}}},RESTMethods=(RESTMethods2=>{RESTMethods2.HEAD="HEAD";RESTMethods2.GET="GET";RESTMethods2.POST=
"POST";RESTMethods2.PUT="PUT";RESTMethods2.PATCH="PATCH";RESTMethods2.OPTIONS="OPTIONS";RESTMethods2.DELETE="DELETE";return RESTMethods2})(RESTMethods||{}),restContext={...defaultContext,cookie,body,text,json,xml},RestRequest=class extends MockedRequest{constructor(request,params){super(request.url,{...request,body:request._body});this.params=params;this.id=request.id}},RestHandler=class extends RequestHandler{constructor(method,path,resolver){super({info:{header:`${method} ${path}`,path,method},
ctx:restContext,resolver});this.checkRedundantQueryParameters()}checkRedundantQueryParameters(){const {method,path}=this.info;if(!(path instanceof RegExp)&&cleanUrl(path)!==path){var queryParams=[];(new URL(`/${path}`,"http://localhost")).searchParams.forEach((_,paramName)=>{queryParams.push(paramName)});devUtils.warn(`Found a redundant usage of query parameters in the request handler URL for "${method} ${path}". Please match against a path instead and access query parameters in the response resolver function using "req.url.searchParams".`)}}parse(request,
resolutionContext){return matchRequestUrl(request.url,this.info.path,null==resolutionContext?void 0:resolutionContext.baseUrl)}getPublicRequest(request,parsedResult){return new RestRequest(request,parsedResult.params||{})}predicate(request,parsedResult){return(this.info.method instanceof RegExp?this.info.method.test(request.method):isStringEqual(this.info.method,request.method))&&parsedResult.matches}log(request,response2){const publicUrl=getPublicUrlFromRequest(request),loggedRequest={...request,
body:request.body,headers:request.headers.all()},loggedResponse=prepareResponse(response2),statusColor=getStatusCodeColor(response2.status);console.groupCollapsed(devUtils.formatMessage("%s %s %s (%c%s%c)"),getTimestamp(),request.method,publicUrl,`color:${statusColor}`,`${response2.status} ${response2.statusText}`,"color:inherit");console.log("Request",loggedRequest);console.log("Handler:",this);console.log("Response",loggedResponse);console.groupEnd()}},import_outvariant3=require("module$node_modules$outvariant$lib$index"),
graphqlContext={...defaultContext,data,extensions,errors,cookie,field:(fieldName,fieldValue)=>res=>{(0,import_outvariant3.invariant)(""!==fieldName.trim(),devUtils.formatMessage("Failed to set a custom field on a GraphQL response: field name cannot be empty."));(0,import_outvariant3.invariant)("data"!==fieldName,devUtils.formatMessage('Failed to set a custom "%s" field on a mocked GraphQL response: forbidden field name. Did you mean to call "ctx.data()" instead?',fieldName));(0,import_outvariant3.invariant)("errors"!==
fieldName,devUtils.formatMessage('Failed to set a custom "%s" field on a mocked GraphQL response: forbidden field name. Did you mean to call "ctx.errors()" instead?',fieldName));(0,import_outvariant3.invariant)("extensions"!==fieldName,devUtils.formatMessage('Failed to set a custom "%s" field on a mocked GraphQL response: forbidden field name. Did you mean to call "ctx.extensions()" instead?',fieldName));var prevBody=jsonParse(res.body)||{};prevBody=mergeRight(prevBody,{[fieldName]:fieldValue});return json(prevBody)(res)}},
GraphQLRequest=class extends MockedRequest{constructor(request,variables,operationName){super(request.url,{...request,body:request._body});this.variables=variables;this.operationName=operationName}},GraphQLHandler=class extends RequestHandler{constructor(operationType,operationName,endpoint,resolver){let resolvedOperationName=operationName;if(null==operationName?0:"object"===typeof operationName&&"kind"in operationName&&"definitions"in operationName){operationName=parseDocumentNode(operationName);
if(operationName.operationType!==operationType)throw Error(`Failed to create a GraphQL handler: provided a DocumentNode with a mismatched operation type (expected "${operationType}", but got "${operationName.operationType}").`);if(!operationName.operationName)throw Error("Failed to create a GraphQL handler: provided a DocumentNode with no operation name.");resolvedOperationName=operationName.operationName}super({info:{header:"all"===operationType?`${operationType} (origin: ${endpoint.toString()})`:
`${operationType} ${resolvedOperationName} (origin: ${endpoint.toString()})`,operationType,operationName:resolvedOperationName},ctx:graphqlContext,resolver});this.endpoint=endpoint}parse(request){return tryCatch(()=>parseGraphQLRequest(request),error2=>console.error(error2.message))}getPublicRequest(request,parsedResult){var _a,_b;return new GraphQLRequest(request,null!=(_a=null==parsedResult?void 0:parsedResult.variables)?_a:{},null!=(_b=null==parsedResult?void 0:parsedResult.operationName)?_b:"")}predicate(request,
parsedResult){if(!parsedResult)return!1;if(!parsedResult.operationName&&"all"!==this.info.operationType)return parsedResult=getPublicUrlFromRequest(request),devUtils.warn(`Failed to intercept a GraphQL request at "${request.method} ${parsedResult}": anonymous GraphQL operations are not supported.
...
this.info.operationName;return request.matches&&hasMatchingOperationType&&parsedResult}log(request,response2,parsedRequest){request={...request,body:request.body,headers:request.headers.all()};const loggedResponse=prepareResponse(response2),statusColor=getStatusCodeColor(response2.status);parsedRequest=(null==parsedRequest?0:parsedRequest.operationName)?`${null==parsedRequest?void 0:parsedRequest.operationType} ${null==parsedRequest?void 0:parsedRequest.operationName}`:`anonymous ${null==parsedRequest?
...
require("module$node_modules$headers_polyfill$lib$index"),createRequestListener=(context,options)=>async(event,message)=>{var _a;const messageChannel=new WorkerChannel(event.ports[0]);event=parseWorkerRequest(message.payload);try{await handleRequest(event,context.requestHandlers,options,context.emitter,{transformResponse,onPassthroughResponse(){messageChannel.postMessage("NOT_FOUND")},async onMockedResponse(response2,{handler,publicRequest,parsedRequest}){if(response2.body instanceof ReadableStream)throw Error(devUtils.formatMessage('Failed to construct a mocked response with a "ReadableStream" body: mocked streams are not supported. Follow https://github.com/mswjs/msw/issues/1336 for more details.'));
const responseBodyBuffer=await (new Response(response2.body,response2)).arrayBuffer();messageChannel.postMessage("MOCK_RESPONSE",{...response2,body:null==response2.body?null:responseBodyBuffer},[responseBodyBuffer]);if(!options.quiet)context.emitter.once("response:mocked",response3=>{handler.log(publicRequest,serializeResponse(response3),parsedRequest)})}})}catch(error2){error2 instanceof NetworkError?messageChannel.postMessage("NETWORK_ERROR",{name:error2.name,message:error2.message}):error2 instanceof
Error&&(devUtils.error('Uncaught exception in the request handler for "%s %s":\n\n%s\n\nThis exception has been gracefully handled as a 500 response, however, it\'s strongly recommended to resolve this error, as it indicates a mistake in your code. If you wish to mock an error response, please see this guide: https://mswjs.io/docs/recipes/mocking-error-responses',event.method,event.url,null!=(_a=error2.stack)?_a:error2),messageChannel.postMessage("MOCK_RESPONSE",{status:500,statusText:"Request Handler Error",
headers:{"Content-Type":"application/json"},body:JSON.stringify({name:error2.name,message:error2.message,stack:error2.stack})}))}},import_until3=require("module$node_modules$$open_draft$until$lib$index"),createStartHandler=context=>function(options,customOptions){const workerRegistration=(async()=>{context.events.removeAllListeners();context.workerChannel.on("REQUEST",createRequestListener(context,options));context.workerChannel.on("RESPONSE",createResponseListener(context));var instance=await getWorkerInstance(options.serviceWorker.url,
options.serviceWorker.options,options.findWorker);const [worker,registration]=instance;if(!worker)throw instance=(null==customOptions?0:customOptions.findWorker)?devUtils.formatMessage('Failed to locate the Service Worker registration using a custom "findWorker" predicate.\n\nPlease ensure that the custom predicate properly locates the Service Worker registration at "%s".\nMore details: https://mswjs.io/docs/api/setup-worker/start#findworker\n',options.serviceWorker.url):devUtils.formatMessage('Failed to locate the Service Worker registration.\n\nThis most likely means that the worker script URL "%s" cannot resolve against the actual public hostname (%s). This may happen if your application runs behind a proxy, or has a dynamic hostname.\n\nPlease consider using a custom "serviceWorker.url" option to point to the actual worker script location, or a custom "findWorker" option to resolve the Service Worker registration manually. More details: https://mswjs.io/docs/api/setup-worker/start',
...
import_fetch3=require("module$node_modules$$mswjs$interceptors$lib$interceptors$fetch$index"),import_XMLHttpRequest=require("module$node_modules$$mswjs$interceptors$lib$interceptors$XMLHttpRequest$index"),import_interceptors3=require("module$node_modules$$mswjs$interceptors$lib$index"),noop=()=>{throw Error("Not implemented");},import_outvariant4=require("module$node_modules$outvariant$lib$index"),import_strict_event_emitter=require("module$node_modules$msw$node_modules$strict_event_emitter$lib$index"),
SetupApi=class{constructor(...initialHandlers){this.validateHandlers(...initialHandlers);this.initialHandlers=toReadonlyArray(initialHandlers);this.currentHandlers=[...initialHandlers];this.emitter=new import_strict_event_emitter.Emitter;this.publicEmitter=new import_strict_event_emitter.Emitter;pipeEvents(this.emitter,this.publicEmitter);this.events=this.createLifeCycleEvents()}validateHandlers(...handlers){for(const handler of handlers)(0,import_outvariant4.invariant)(!Array.isArray(handler),devUtils.formatMessage('Failed to construct "%s" given an Array of request handlers. Make sure you spread the request handlers when calling the respective setup function.'),
this.constructor.name)}dispose(){this.emitter.removeAllListeners();this.publicEmitter.removeAllListeners()}use(...runtimeHandlers){this.currentHandlers.unshift(...runtimeHandlers)}restoreHandlers(){this.currentHandlers.forEach(handler=>{handler.markAsSkipped(!1)})}resetHandlers(...nextHandlers){this.currentHandlers=0<nextHandlers.length?[...nextHandlers]:[...this.initialHandlers]}listHandlers(){return toReadonlyArray(this.currentHandlers)}createLifeCycleEvents(){return{on:(...args)=>this.publicEmitter.on(...args),
removeListener:(...args)=>this.publicEmitter.removeListener(...args),removeAllListeners:(...args)=>this.publicEmitter.removeAllListeners(...args)}}},SetupWorkerApi=class extends SetupApi{constructor(...handlers){super(...handlers);this.stopHandler=this.startHandler=null;(0,import_outvariant5.invariant)(!(0,import_is_node_process3.isNodeProcess)(),devUtils.formatMessage("Failed to execute `setupWorker` in a non-browser environment. Consider using `setupServer` for Node.js environment instead."));this.listeners=
[];this.context=this.createWorkerContext()}createWorkerContext(){const context={isMockingEnabled:!1,startOptions:null,worker:null,registration:null,requestHandlers:this.currentHandlers,emitter:this.emitter,workerChannel:{on:(eventType,callback)=>{this.context.events.addListener(navigator.serviceWorker,"message",event=>{if(event.source===this.context.worker){var message=event.data;message&&message.type===eventType&&callback(event,message)}})},send:type=>{var _a;null==(_a=this.context.worker)?void 0:
_a.postMessage(type)}},events:{addListener:(target,eventType,callback)=>{target.addEventListener(eventType,callback);this.listeners.push({eventType,target,callback});return()=>{target.removeEventListener(eventType,callback)}},removeAllListeners:()=>{for(const {target,eventType,callback}of this.listeners)target.removeEventListener(eventType,callback);this.listeners=[]},once:eventType=>{const bindings=[];return(new Promise((resolve,reject)=>{bindings.push(this.context.events.addListener(navigator.serviceWorker,
"message",event=>{try{const message=event.data;message.type===eventType&&resolve(message)}catch(error2){reject(error2)}}),this.context.events.addListener(navigator.serviceWorker,"messageerror",reject))})).finally(()=>{bindings.forEach(unbind=>unbind())})}},useFallbackMode:!("serviceWorker"in navigator)||"file:"===location.protocol};Object.defineProperties(context,{requestHandlers:{get:()=>this.currentHandlers}});this.startHandler=context.useFallbackMode?createFallbackStart(context):createStartHandler(context);
this.stopHandler=context.useFallbackMode?createFallbackStop(context):createStop(context);return context}async start(options={}){this.context.startOptions=mergeRight(DEFAULT_START_OPTIONS,options);return await this.startHandler(this.context.startOptions,options)}printHandlers(){this.listHandlers().forEach(handler=>{const {header,callFrame}=handler.info,pragma=handler.info.hasOwnProperty("operationType")?"[graphql]":"[rest]";console.groupCollapsed(`${pragma} ${header}`);callFrame&&console.log(`Declaration: ${callFrame}`);
console.log("Handler:",handler);console.groupEnd()})}stop(){super.dispose();this.context.events.removeAllListeners();this.context.emitter.removeAllListeners();this.stopHandler()}},rest={all:createRestHandler(/.+/),head:createRestHandler("HEAD"),get:createRestHandler("GET"),post:createRestHandler("POST"),put:createRestHandler("PUT"),delete:createRestHandler("DELETE"),patch:createRestHandler("PATCH"),options:createRestHandler("OPTIONS")},graphql={operation:createGraphQLOperationHandler("*"),query:createScopedGraphQLHandler("query",
"*"),mutation:createScopedGraphQLHandler("mutation","*"),link:function(url){return{operation:createGraphQLOperationHandler(url),query:createScopedGraphQLHandler("query",url),mutation:createScopedGraphQLHandler("mutation",url)}}};(0,import_outvariant2.invariant)("undefined"!==typeof URL,devUtils.formatMessage('Global "URL" class is not defined. This likely means that you\'re running MSW in an environment that doesn\'t support all Node.js standard API (e.g. React Native). If that\'s the case, please use an appropriate polyfill for the "URL" class, like "react-native-url-polyfill".'))}
